...
final class LdapClient implements LdapClientInterface
{
    ...
    public function __construct($host = null, $port = 389, $version = 3, $useSsl = false, $useStartTls = false, $optReferrals = false, LdapInterface $ldap = null)
    {
        ...
        $this->ldap = $ldap instanceof \Symfony\Component\Ldap\LdapInterface ? $ldap : Ldap::create('ext_ldap', $config);
    }
     ...
     * {@inheritdoc}
     */
    public function find($dn, $query, $filter = '*'): array
    {
    ...
    private function normalizeConfig($host, $port, $version, $useSsl, $useStartTls, $optReferrals): array
    {
        if ((bool) $useSsl) {
            $encryption = 'ssl';
