...
class Filesystem
{
     ...
     */
    public function mkdir(string|iterable $dirs, $mode = 0777): void
    {
        foreach ($this->toIterable($dirs) as $dir) {
            ...
            if (!$this->box('mkdir', $dir, $mode, true) && !is_dir($dir)) {
     ...
     */
    public function touch(string|iterable $files, $time = null, $atime = null): void
    {
        foreach ($this->toIterable($files) as $file) {
    ...
    public function remove($files): void
    {
        ...
        foreach ($files as $file) {
            if (is_link($file)) {
                ...
                if (!($this->box('unlink', $file) || '\\' !== \DIRECTORY_SEPARATOR || $this->box('rmdir', $file)) && file_exists($file)) {
                    throw new IOException(sprintf('Failed to remove symlink "%s": ', $file).self::$lastError);
                }
            } elseif (is_dir($file)) {
                $this->remove(new \FilesystemIterator($file, \FilesystemIterator::CURRENT_AS_PATHNAME | \FilesystemIterator::SKIP_DOTS));

                if (!$this->box('rmdir', $file) && file_exists($file)) {
                    throw new IOException(sprintf('Failed to remove directory "%s": ', $file).self::$lastError);
                }
            } elseif (!$this->box('unlink', $file) && (false !== strpos(self::$lastError, 'Permission denied') || file_exists($file))) {
     ...
     */
    public function chmod(string|iterable $files, $mode, $umask = 0000, $recursive = false): void
    {
        foreach ($this->toIterable($files) as $file) {
            if ((\PHP_VERSION_ID < 80000 || \is_int($mode)) && !@chmod($file, $mode & ~$umask)) {
     ...
     * @throws IOException When the change fails
     */
    public function chown(string|iterable $files, $user, $recursive = false): void
    {
        foreach ($this->toIterable($files) as $file) {
            if ($recursive && is_dir($file) && !is_link($file)) {
     ...
     * @throws IOException When the change fails
     */
    public function chgrp(string|iterable $files, $group, $recursive = false): void
    {
        foreach ($this->toIterable($files) as $file) {
            if ($recursive && is_dir($file) && !is_link($file)) {
            ...
            if (is_link($file) && \function_exists('lchgrp')) {
                if (!@lchgrp($file, $group) || (\defined('HHVM_VERSION') && !posix_getgrnam($group))) {
    ...
    public function symlink($originDir, $targetDir, $copyOnWindows = false): void
    {
        ...
        if (!$this->box('symlink', $originDir, $targetDir)) {
     ...
     */
    public function hardlink($originFile, string|iterable $targetFiles): void
    {
        ...
        foreach ($this->toIterable($targetFiles) as $targetFile) {
            ...
            if (!$this->box('link', $originFile, $targetFile)) {
     ...
     */
    private function linkException($origin, $target, string $linkType): void
    {
        if (self::$lastError && ('\\' === \DIRECTORY_SEPARATOR && false !== strpos(self::$lastError, 'error code(1314)'))) {
    ...
    public function makePathRelative($endPath, $startPath): string
    {
        ...
        $splitDriveLetter = static fn($path): array => (\strlen($path) > 2 && ':' === $path[1] && '/' === $path[2] && ctype_alpha($path[0]))
            ? [substr($path, 2), strtoupper($path[0])]
            : [$path, null];

        $splitPath = static function ($path, $absolute) : array {
            $result = [];
     ...
     */
    public function mirror($originDir, $targetDir, \Traversable $iterator = null, array $options = []): void
    {
     ...
     *
     */
    private function toIterable(string|iterable $files): \Traversable|array
    {
     ...
     * @return mixed
     */
    private function box(string $func)
    {
        self::$lastError = null;
     ...
     * @internal
     */
    public static function handleError($type, ?string $msg): void
    {
        self::$lastError = $msg;
    }
}
