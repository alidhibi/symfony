...
class Request
{
    ...
    /** @deprecated since version 3.3, to be removed in 4.0 */
    private static array $trustedHeaderNames = [
        self::HEADER_FORWARDED => 'FORWARDED',
        self::HEADER_CLIENT_IP => 'X_FORWARDED_FOR',
        self::HEADER_CLIENT_HOST => 'X_FORWARDED_HOST',
        self::HEADER_CLIENT_PROTO => 'X_FORWARDED_PROTO',
     ...
     * @return static
     */
    public static function create($uri, $method = 'GET', $parameters = [], array $cookies = [], array $files = [], array $server = [], $content = null)
    {
    ...
    public function overrideGlobals(): void
    {
        ...
        foreach (str_split($requestOrder) as $order) {
            $_REQUEST = [...$_REQUEST, ...$request[$order]];
        }
    }
    ...
    public static function setTrustedProxies(array $proxies/*, int $trustedHeaderSet*/): void
    {
        ...
        foreach (self::$trustedHeaderNames as $header => $name) {
            self::$trustedHeaders[$header] = ($header & $trustedHeaderSet) !== 0 ? $name : null;
        }
    ...
    {
        self::$trustedHostPatterns = array_map(static fn($hostPattern): string => sprintf('{%s}i', $hostPattern), $hostPatterns);
    ...
    public function isMethodSafe(/* $andCacheable = true */)
    {
        if (!\func_num_args() || func_get_arg(0)) {
            ...
            @trigger_error('Checking only for cacheable HTTP methods with ' . \Symfony\Component\HttpFoundation\Request::class . '::isMethodSafe() is deprecated since Symfony 3.2 and will throw an exception in 4.0. Disable checking only for cacheable methods by calling the method with `false` as first argument or use the Request::isMethodCacheable() instead.', \E_USER_DEPRECATED);
    ...
    private function getTrustedValues(int $type, $ip = null)
    {
        $clientValues = [];
        $forwardedValues = [];
