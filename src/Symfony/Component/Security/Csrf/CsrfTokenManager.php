...
{
    private readonly \Symfony\Component\Security\Csrf\TokenGenerator\TokenGeneratorInterface $generator;

    private readonly \Symfony\Component\Security\Csrf\TokenStorage\TokenStorageInterface $storage;
    ...
    public function __construct(TokenGeneratorInterface $generator = null, TokenStorageInterface $storage = null, $namespace = null)
    {
        ...
        $superGlobalNamespaceGenerator = static fn(): string => !empty($_SERVER['HTTPS']) && 'off' !== strtolower($_SERVER['HTTPS']) ? 'https-' : '';
        ...
        if (null === $namespace) {
            ...
            $this->namespace = static function () use ($namespace, $superGlobalNamespaceGenerator) : string {
                if (($request = $namespace->getMasterRequest()) !== null) {
