...
class OptionsResolver implements Options
{
    ...
    public function resolve(array $options = [])
    {
        ...
        foreach (array_keys($clone->defaults) as $option) {
        ...
        if (isset($this->allowedTypes[$option])) {
            ...
            if (!$valid) {
                ...
                $allowedContainsArrayType = \count(array_filter(
                    ...
                    static fn($item): bool => '[]' === substr(isset(self::$typeAliases[$item]) ? self::$typeAliases[$item] : $item, -2)
                )) > 0;
    ...
    private function verifyTypes($type, $value, array &$invalidTypes)
    {
        ...
        if ($this->isValueValidType($type, $value)) {
            return true;
        }
     ...
     * @return bool
     */
    private function verifyArrayType(string $type, array $value, array &$invalidTypes, int|float $level = 0)
    {
        ...
        foreach ($value as $item) {
            if (!$this->isValueValidType($type, $item)) {
     ...
     */
    private function formatTypeOf($value, ?string $type)
    {
        $suffix = '';
    ...
    private function isValueValidType(string $type, $value)
    {
    ...
    private function getParameterClassName(\ReflectionParameter $parameter)
    {
        if (!method_exists($parameter, 'getType')) {
            return (($class = $parameter->getClass()) !== null) ? $class->name : null;
        }
