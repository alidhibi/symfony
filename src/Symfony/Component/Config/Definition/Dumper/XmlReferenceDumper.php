...
class XmlReferenceDumper
{
    ...
    public function dumpNode(NodeInterface $node, $namespace = null): string
    {
        $this->reference = '';
     ...
     */
    private function writeNode(NodeInterface $node, int|float $depth = 0, bool $root = false, $namespace = null): void
    {
        ...
        // xml remapping
        if ($node->getParent()) {
            $remapping = array_filter($node->getParent()->getXmlRemappings(), static fn($mapping): bool => $rootName === $mapping[1]);

            if (\count($remapping) > 0) {
        ...
        if ($node instanceof ArrayNode) {
            ...
            if ($node instanceof PrototypedArrayNode) {
                ...
                } elseif ($prototype->hasDefaultValue()) {
                    $prototypeValue = $prototype->getDefaultValue();
                } else {
                    switch (\get_class($prototype)) {
                        case \Symfony\Component\Config\Definition\ScalarNode::class:
                            $prototypeValue = 'scalar value';
                            break;

                        case \Symfony\Component\Config\Definition\FloatNode::class:
                        case \Symfony\Component\Config\Definition\IntegerNode::class:
                            $prototypeValue = 'numeric value';
                            break;

                        case \Symfony\Component\Config\Definition\BooleanNode::class:
                            $prototypeValue = 'true|false';
                            break;

                        case \Symfony\Component\Config\Definition\EnumNode::class:
     ...
     * @param int    $indent
     */
    private function writeLine($text, $indent = 0): void
    {
     ...
     *
     */
    private function writeValue($value): string|float|int
    {
