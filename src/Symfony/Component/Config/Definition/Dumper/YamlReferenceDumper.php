...
class YamlReferenceDumper
{
    ...
    private function writeNode(NodeInterface $node, NodeInterface $parentNode = null, int|float $depth = 0, bool $prototypedArray = false): void
    {
        $comments = [];
        $default = '';
        $defaultArray = null;
        ...
        if ($node instanceof ArrayNode) {
            ...
            if ($node->hasDefaultValue()) {
                ...
                if (\is_array($default)) {
                    if (\count($defaultArray = $node->getDefaultValue()) > 0) {
                        $default = '';
                    } elseif (!\is_array($example)) {
                        $default = '[]';
                    }
        ...
        if ($node->isDeprecated()) {
            $comments[] = sprintf('Deprecated (%s)', $node->getDeprecationMessage($node->getName(), $parentNode instanceof \Symfony\Component\Config\Definition\NodeInterface ? $parentNode->getPath() : $node->getPath()));
        }
     ...
     * @param string $text
     */
    private function writeLine($text, int|float $indent = 0): void
    {
    ...
    private function writeArray(array $array, int|float $depth): void
    {
