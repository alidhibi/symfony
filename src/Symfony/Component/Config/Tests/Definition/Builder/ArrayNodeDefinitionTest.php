...
class ArrayNodeDefinitionTest extends TestCase
{
     ...
     */
    public function testPrototypedArrayNodeDefault(int|string|array|null $args, bool $shouldThrowWhenUsingAttrAsKey, bool $shouldThrowWhenNotUsingAttrAsKey, array $defaults): void
    {
        $node = new ArrayNodeDefinition('root');
        $node
    ...
    public function testUnsetChild(): void
    {
        ...
        $node
            ->children()
                ->scalarNode('value')
                    ->beforeNormalization()
                        ->ifTrue(static fn($value): bool => empty($value))
                        ->thenUnset()
                    ->end()
                ->end()
            ->end()
