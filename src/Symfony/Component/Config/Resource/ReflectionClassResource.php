...
class ReflectionClassResource implements SelfCheckingResourceInterface, \Serializable
{
    ...
    private function computeHash(): bool|string
    {
        if (!$this->classReflector instanceof \ReflectionClass) {
            try {
            ...
            foreach ($class->getMethods(\ReflectionMethod::IS_PUBLIC | \ReflectionMethod::IS_PROTECTED) as $m) {
                ...
                if ($parametersWithUndefinedConstants === []) {
                    ...
                    $stack = [
                        ...
                        $m->isProtected(),
                        $m->returnsReference(),
                        $t instanceof \ReflectionNamedType ? ($t->allowsNull()).$t->getName() : (string) $t,
                    ];
                    ...
                    foreach ($m->getParameters() as $p) {
                        if (!isset($parametersWithUndefinedConstants[$p->name])) {
                            ...
                            $stack[] = $p->isOptional();
                            $stack[] = $t instanceof \ReflectionNamedType ? ($t->allowsNull()).$t->getName() : (string) $t;
...
{
    /**
     * @return \Symfony\Component\Config\Resource\ReflectionParameterHhvmWrapper[]
     */
    public function getParameters(): array
    {
        $params = [];

        foreach (array_keys(parent::getParameters()) as $i) {
