...
class PropertyAccessor implements PropertyAccessorInterface
{
     ...
     * @internal
     */
    final const IS_REF_CHAINED = 2;

    /**
     * @internal
     */
    final const ACCESS_HAS_PROPERTY = 0;

    /**
     * @internal
     */
    final const ACCESS_TYPE = 1;

    /**
     * @internal
     */
    final const ACCESS_NAME = 2;

    /**
     * @internal
     */
    final const ACCESS_REF = 3;

    /**
     * @internal
     */
    final const ACCESS_ADDER = 4;

    /**
     * @internal
     */
    final const ACCESS_REMOVER = 5;

    /**
     * @internal
     */
    final const ACCESS_TYPE_METHOD = 0;

    /**
     * @internal
     */
    final const ACCESS_TYPE_PROPERTY = 1;

    /**
     * @internal
     */
    final const ACCESS_TYPE_MAGIC = 2;

    /**
     * @internal
     */
    final const ACCESS_TYPE_ADDER_AND_REMOVER = 3;

    /**
     * @internal
     */
    final const ACCESS_TYPE_NOT_FOUND = 4;

    /**
     * @internal
     */
    final const CACHE_PREFIX_READ = 'r';

    /**
     * @internal
     */
    final const CACHE_PREFIX_WRITE = 'w';

    /**
     * @internal
     */
    final const CACHE_PREFIX_PROPERTY_PATH = 'p';
    ...
    private static array $errorHandler = [__CLASS__, 'handleError'];
    ...
    public function setValue(&$objectOrArray, $propertyPath, $value): void
    {
        ...
        try {
            ...
            self::throwInvalidArgumentException($typeError->getMessage(), $typeError->getTrace(), 0, $typeError);
     ...
     * @internal
     */
    public static function handleError($type, $message, $file, $line, $context = []): bool
    {
    ...
    private static function throwInvalidArgumentException($message, array $trace, $i, $previous = null): void
    {
     ...
     */
    private function readPropertiesUntil(array $zval, PropertyPathInterface $propertyPath, $lastIndex, bool $ignoreInvalidIndices = true): array
    {
     ...
     */
    private function readIndex($zval, $index): array
    {
        if (!$zval[self::VALUE] instanceof \ArrayAccess && !\is_array($zval[self::VALUE])) {
     ...
     */
    private function readProperty(array $zval, $property): array
    {
        if (!\is_object($zval[self::VALUE])) {
        ...
        try {
            ...
            if (\PHP_VERSION_ID >= 70400 && preg_match('/^Typed property ([\w\\\]+)::\$(\w+) must not be accessed before initialization$/', $error->getMessage(), $matches)) {
                $r = new \ReflectionProperty($matches[1], $matches[2]);
                $type = ($type = $r->getType()) instanceof \ReflectionNamedType ? $type->getName() : (string) $type;

                throw new AccessException(sprintf('The property "%s::$%s" is not readable because it is typed "%s". You should initialize it or declare a default value instead.', $r->getDeclaringClass()->getName(), $r->getName(), $type), 0, $error);
            }
     ...
     * @throws NoSuchIndexException If the array does not implement \ArrayAccess or it is not an array
     */
    private function writeIndex($zval, $index, $value): void
    {
        if (!$zval[self::VALUE] instanceof \ArrayAccess && !\is_array($zval[self::VALUE])) {
     ...
     * @throws NoSuchPropertyException if the property does not exist or is not public
     */
    private function writeProperty(array $zval, $property, $value): void
    {
        if (!\is_object($zval[self::VALUE])) {
     ...
     */
    private function writeCollection(array $zval, $property, $collection, $addMethod, $removeMethod): void
    {
     ...
     * @return array
     */
    private function getWriteAccessInfo(string $class, string $property, $value)
    {
        ...
        if (!isset($access[self::ACCESS_TYPE])) {
            ...
            if ($this->isMethodAccessible($reflClass, $setter, 1)) {
                ...
                $access[self::ACCESS_NAME] = sprintf(
                    ...
                    $property,
                    implode('', array_map(static fn($singular): string => '"add'.$singular.'()"/"remove'.$singular.'()", ', $singulars)),
                    $setter,
                    $getsetter,
                    $reflClass->name
                );
     ...
     */
    private function isPropertyWritable($object, $property): bool
    {
        if (!\is_object($object)) {
            return false;
        }
     ...
     */
    public static function createCache($namespace, $defaultLifetime, $version, LoggerInterface $logger = null): \Symfony\Component\Cache\Adapter\NullAdapter|\Symfony\Component\Cache\Adapter\ApcuAdapter
    {
