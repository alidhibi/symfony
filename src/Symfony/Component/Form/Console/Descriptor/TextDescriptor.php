...
class TextDescriptor extends Descriptor
{
    protected function describeDefaults(array $options)
    {
        ...
        $shortClassNames = array_map(static fn($fqcn) => \array_slice(explode('\\', $fqcn), -1)[0], $options['core_types']);
        $counter = \count($shortClassNames);
    ...
    protected function describeOption(OptionsResolver $optionsResolver, array $options)
    {
        ...
        foreach ($map as $label => $name) {
            ...
            $rows[] = [sprintf('<info>%s</info>', $label), $value];
    ...
    private function getDumpFunction()
    {
        ...
        $cloner->addCasters(['Closure' => static function ($c, array $a) : array {
        ...
        return static fn($value): string => rtrim($dumper->dump($cloner->cloneVar($value)->withRefHandles(false), true));
    }
}
