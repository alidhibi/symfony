...
class FormValidatorTest extends ConstraintValidatorTestCase
{
    ...
    {
        $object = new \stdClass();
        $options = ['validation_groups' => fn(\Symfony\Component\Form\FormInterface $form) => $this->getValidationGroups($form)];
        $form = $this->getCompoundForm($object, $options);
        $form->submit([]);
    ...
    {
        $object = new \stdClass();
        $options = ['validation_groups' => static fn(FormInterface $form): array => ['group1', 'group2']];
        $form = $this->getCompoundForm($object, $options);
        $form->submit([]);
    ...
    public function testUseInheritedCallbackValidationGroup(): void
    {
        ...
        $parentOptions = ['validation_groups' => fn(\Symfony\Component\Form\FormInterface $form) => $this->getValidationGroups($form)];
        $parent = $this->getBuilder('parent', null, $parentOptions)
            ->setCompound(true)
            ->setDataMapper(new PropertyPathMapper())
            ->getForm();
    ...
    public function testUseInheritedClosureValidationGroup(): void
    {
        ...
        $parentOptions = [
            'validation_groups' => static fn(): array => ['group1', 'group2'],
        ];
        $parent = $this->getBuilder('parent', null, $parentOptions)
            ->setCompound(true)
            ->setDataMapper(new PropertyPathMapper())
    ...
    protected function createValidator(): \Symfony\Component\Form\Extension\Validator\Constraints\FormValidator
    {
     ...
     *
     */
    private function getBuilder($name = 'name', $dataClass = null, array $options = []): \Symfony\Component\Form\FormBuilder
    {
    ...
    private function getForm(string $name = 'name', $dataClass = null, array $options = [])
    {
        return $this->getBuilder($name, $dataClass, $options)->getForm();
    }

    private function getCompoundForm(\stdClass $data, array $options = [])
    {
    ...
    private function getSubmitButton(string $name = 'name', array $options = [])
    {
