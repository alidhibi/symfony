...
{
    final const LEVEL_0 = 0;

    final const LEVEL_1 = 1;

    final const LEVEL_1B = 2;

    final const LEVEL_2 = 3;
    ...
    private \Symfony\Component\Form\Extension\Validator\ViolationMapper\ViolationMapper $mapper;
    ...
    protected function getForm($name = 'name', $propertyPath = null, $dataClass = null, $errorMapping = [], $inheritData = false, $synchronized = true, array $options = []): \Symfony\Component\Form\Form
    {
     ...
     *
     */
    protected function getConstraintViolation($propertyPath, $root = null): \Symfony\Component\Validator\ConstraintViolation
    {
        return new ConstraintViolation($this->message, $this->messageTemplate, $this->params, $root, $propertyPath, null);
    }

    protected function getFormError(ConstraintViolationInterface $violation, FormInterface $form): \Symfony\Component\Form\FormError
    {
    ...
    public function provideDefaultTests(): array
    {
     ...
     */
    public function testDefaultErrorMapping(int $target, string $childName, string $childPath, string $grandChildName, string $grandChildPath, string $violationPath): void
    {
    ...
    public function provideCustomDataErrorTests(): array
    {
        return [
     ...
     */
    public function testCustomDataErrorMapping(int $target, string $mapFrom, string $mapTo, string $childName, string $childPath, string $grandChildName, string $grandChildPath, string $violationPath): void
    {
        $violation = $this->getConstraintViolation($violationPath);
    ...
    public function provideCustomFormErrorTests(): array
    {
     ...
     */
    public function testCustomFormErrorMapping(int $target, string $mapFrom, string $mapTo, string $errorName, string $errorPath, string $childName, string $childPath, string $grandChildName, string $grandChildPath, string $violationPath): void
    {
        $violation = $this->getConstraintViolation($violationPath);
        $parent = $this->getForm('parent', null, null, [$mapFrom => $mapTo]);
        $child = $this->getForm($childName, $childPath);
    ...
    public function provideErrorTestsForFormInheritingParentData(): array
    {
        return [
            // mapping target, child name, its property path, grand child name, its property path, violation path
            [self::LEVEL_2, 'address', 'address', 'street', 'street', 'children[address].children[street].data'],
     ...
     */
    public function testErrorMappingForFormInheritingParentData(int $target, string $childName, string $childPath, string $grandChildName, string $grandChildPath, string $violationPath): void
    {
        $violation = $this->getConstraintViolation($violationPath);
        $parent = $this->getForm('parent');
