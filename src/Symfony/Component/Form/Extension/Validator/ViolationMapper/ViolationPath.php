...
class ViolationPath implements \IteratorAggregate, PropertyPathInterface
{
     ...
     *
     */
    public function getIterator(): \Symfony\Component\Form\Extension\Validator\ViolationMapper\ViolationPathIterator
    {
    ...
    private function buildString(): void
    {
        ...
        foreach ($this->elements as $index => $element) {
            if ($this->mapsForm[$index]) {
                $this->pathAsString .= sprintf('.children[%s]', $element);
            } elseif (!$data) {
                $this->pathAsString .= '.data'.($this->isIndex[$index] ? sprintf('[%s]', $element) : '.' . $element);
                $data = true;
            } else {
                $this->pathAsString .= $this->isIndex[$index] ? sprintf('[%s]', $element) : '.' . $element;
            }
        }
