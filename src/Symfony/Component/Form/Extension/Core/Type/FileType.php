...
{
    final const KIB_BYTES = 1024;

    final const MIB_BYTES = 1_048_576;
    ...
    public function configureOptions(OptionsResolver $resolver): void
    {
        ...
        if (class_exists(\Symfony\Component\HttpFoundation\File\File::class)) {
            $dataClass = static fn(Options $options): ?string => $options['multiple'] ? null : \Symfony\Component\HttpFoundation\File\File::class;
        }
    ...
    private function getFileUploadError($errorCode): \Symfony\Component\Form\FormError
    {
        $messageParameters = [];

        if (\UPLOAD_ERR_INI_SIZE === $errorCode) {
            list($limitAsString, $suffix) = $this->factorizeSizes(0, $this->getMaxFilesize());
     ...
     */
    private function factorizeSizes(int $size, $limit): array
    {
        $coef = self::MIB_BYTES;
        ...
        while ($this->moreDecimalsThan($limitAsString, 2)) {
            $coef /= $coefFactor;
            $limitAsString = (string) ($limit / $coef);
        }
     ...
     */
    private function moreDecimalsThan($double, int $numberOfDecimals): bool
    {
