...
class CommandTest extends TestCase
{
     ...
     */
    public function testInvalidCommandNames(string $name): void
    {
    ...
    {
        $command = new \TestCommand();
        $ret = $command->setCode(static function (InputInterface $input, OutputInterface $output) : void {
     ...
     */
    public function testSetCodeBindToClosure(bool $previouslyBound, string $expected): void
    {
        $code = createClosure();
        if ($previouslyBound) {
    ...
    {
        $command = new \TestCommand();
        $command->setCode($this->createClosure());
    ...
    {
        $command = new \TestCommand();
        $ret = $command->setCode(fn(\Symfony\Component\Console\Input\InputInterface $input, \Symfony\Component\Console\Output\OutputInterface $output) => $this->callableMethodCommand($input, $output));
        $this->assertEquals($command, $ret, '->setCode() implements a fluent interface');
        $tester = new CommandTester($command);
        $tester->execute([]);
        $this->assertEquals('interact called'.\PHP_EOL.'from the code...'.\PHP_EOL, $tester->getDisplay());
