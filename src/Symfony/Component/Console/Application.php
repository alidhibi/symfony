...
class Application
{
    ...
    private \Symfony\Component\Console\CommandLoader\CommandLoaderInterface $commandLoader;

    private bool $catchExceptions = true;

    private bool $autoExit = true;

    private ?\Symfony\Component\Console\Input\InputDefinition $definition = null;
    ...
    public function run(InputInterface $input = null, OutputInterface $output = null)
    {
        ...
        $renderException = function ($e) use ($output): void {
        ...
        if (($phpHandler = set_exception_handler($renderException)) !== null) {
        ...
        if ($this->dispatcher instanceof \Symfony\Contracts\EventDispatcher\EventDispatcherInterface && $this->dispatcher->hasListeners(ConsoleEvents::EXCEPTION)) {
            @trigger_error('The "ConsoleEvents::EXCEPTION" event is deprecated since Symfony 3.3 and will be removed in 4.0. Listen to the "ConsoleEvents::ERROR" event instead.', \E_USER_DEPRECATED);
        }
    ...
    {
        if (!$this->definition instanceof \Symfony\Component\Console\Input\InputDefinition) {
     ...
     */
    public function add(Command $command): ?\Symfony\Component\Console\Command\Command
    {
        $this->init();
    ...
    public function findNamespace($namespace)
    {
        ...
        $expr = preg_replace_callback('{([^:]+|)}', static fn($matches): string => preg_quote($matches[1]).'[^:]*', $namespace);
        $namespaces = preg_grep('{^'.$expr.'}', $allNamespaces);

        if ($namespaces === [] || $namespaces === false) {
    ...
    public function find($name)
    {
        ...
        $allCommands = $this->commandLoader ? [...$this->commandLoader->getNames(), ...array_keys($this->commands)] : array_keys($this->commands);
        $expr = preg_replace_callback('{([^:]+|)}', static fn($matches): string => preg_quote($matches[1]).'[^:]*', $name);
        ...
        if ($commands === [] || $commands === false || \count(preg_grep('{^'.$expr.'$}i', $commands)) < 1) {
            ...
            if ($alternatives = $this->findAlternatives($name, $allCommands)) {
                ...
                $alternatives = array_filter($alternatives, fn($name): bool => !$this->get($name)->isHidden());
        ...
        if (\count($commands) > 1) {
            $commandList = $this->commandLoader ? [...array_flip($this->commandLoader->getNames()), ...$this->commands] : $this->commands;
        ...
        if (\count($commands) > 1 && !$exact) {
            ...
            $abbrevs = array_map(static function ($cmd) use ($commandList, $usableWidth, $maxLen) : bool|string {
    ...
    protected function doRenderException(\Exception $e, OutputInterface $output)
    {
        do {
            ...
            $width = $this->terminal->getWidth() !== 0 ? $this->terminal->getWidth() - 1 : \PHP_INT_MAX;
            ...
            if (OutputInterface::VERBOSITY_VERBOSE <= $output->getVerbosity()) {
                ...
                for ($i = 0, $count = \count($trace); $i < $count; ++$i) {
                    ...
                    $output->writeln(sprintf(' %s%s at <info>%s:%s</info>', $class, $function !== '' && $function !== '0' ? $type.$function.'()' : '', $file, $line), OutputInterface::VERBOSITY_QUIET);
                }
     ...
     */
    public function setTerminalDimensions($width, $height): static
    {
    ...
    protected function configureIO(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($input->hasParameterOption(['--quiet', '-q'], true)) {
            ...
            $shellVerbosity = -1;
        } elseif ($input->hasParameterOption('-vvv', true) || $input->hasParameterOption('--verbose=3', true) || 3 === $input->getParameterOption('--verbose', false, true)) {
    ...
    private function findAlternatives($name, $collection): array
    {
        ...
        $alternatives = array_filter($alternatives, static fn($lev): bool => $lev < 2 * $threshold);
     ...
     *
     */
    public function setDefaultCommand(string $commandName, $isSingleCommand = false): static
    {
    ...
    private function splitStringByWidth(string $string, $width): array
    {
    ...
    private function extractAllNamespaces($name): array
    {
        ...
        foreach ($parts as $part) {
            $namespaces[] = \count($namespaces) ? end($namespaces).':'.$part : $part;
        }
