...
class ErrorListener implements EventSubscriberInterface
{
    ...
    private function getInputString(ConsoleEvent $event)
    {
        $commandName = $event->getCommand() instanceof \Symfony\Component\Console\Command\Command ? $event->getCommand()->getName() : null;
        $input = $event->getInput();

        if (method_exists($input, '__toString')) {
            if ($commandName) {
                return str_replace([sprintf('\'%s\'', $commandName), sprintf('"%s"', $commandName)], $commandName, (string) $input);
            }
