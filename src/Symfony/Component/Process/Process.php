...
class Process implements \IteratorAggregate
{
    ...
    private string|float|null $lastOutputTime = null;

    private ?float $timeout = null;

    private ?float $idleTimeout = null;

    private array $options = ['suppress_errors' => true];

    private $exitcode;

    private $fallbackStatus = [];

    private array|float|int|null $processInformation = null;
    ...
    private bool $enhanceWindowsCompatibility = true;

    private bool $enhanceSigchildCompatibility;

    private $process;

    private string $status = self::STATUS_READY;

    private int|bool $incrementalOutputOffset = 0;

    private int|bool $incrementalErrorOutputOffset = 0;
    ...
    private bool $useFileHandles = false;

    private \Symfony\Component\Process\Pipes\WindowsPipes|\Symfony\Component\Process\Pipes\UnixPipes|null $processPipes = null;
     ...
     */
    public function __construct(array $commandline, $cwd = null, array $env = null, mixed $input = null, ?float $timeout = 60, array $options = null)
    {
    ...
    public function start(callable $callback = null/*, array $env = [*/): void
    {
        ...
        $this->lastOutputTime = $this->starttime;
        ...
        if (\is_array($commandline = $this->commandline)) {
            $commandline = implode(' ', array_map(fn(?string $argument): string => $this->escapeArgument($argument), $commandline));
    ...
    public function getIterator($flags = 0)
    {
        ...
        while ($this->callback instanceof \Closure || ($yieldOut && !feof($this->stdout)) || ($yieldErr && !feof($this->stderr))) {
            if ($yieldOut) {
     ...
     */
    public function addOutput($line): void
    {
    ...
    {
        return \is_array($this->commandline) ? implode(' ', array_map(fn(?string $argument): string => $this->escapeArgument($argument), $this->commandline)) : $this->commandline;
    }
     ...
     * @return $this
     */
    public function setCommandLine(array $commandline)
    {
        $this->commandline = $commandline;
    ...
    public function setEnv(array $env)
    {
        ...
        $env = array_filter($env, static fn($value): bool => !\is_array($value));
    ...
    protected function buildCallback(callable $callback = null)
    {
        if ($this->outputDisabled) {
            return static function ($type, $data) use ($callback) : void {
                if (null !== $callback) {
        ...
        return function ($type, $data) use ($callback, $out): void {
            if ($out == $type) {
                $this->addOutput($data);
            } else {
     ...
     */
    private function readPipes($blocking, bool $close): void
    {
     ...
     * @throws RuntimeException In case of failure
     */
    private function doSignal($signal, bool $throwException)
    {
    ...
    private function prepareWindowsCommandLine(string $cmd, array &$env)
    {
        ...
        $cmd = preg_replace_callback(
            ...
            static function (array $m) use (&$env, &$varCache, &$varCount, $uid) {
                if (!isset($m[1])) {
                    return $m[0];
                }
