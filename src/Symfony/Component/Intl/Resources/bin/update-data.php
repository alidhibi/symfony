...
$gitTag = $git->getLastTag(static fn($tag): int|false => preg_match('#^release-\d{1,}-\d{1}$#', $tag));
$shortIcuVersion = strip_minor_versions(preg_replace('#release-(\d{1,})-(\d{1,})#', '$1.$2', $gitTag));
...
file_put_contents($versionFile, $icuVersionInDownload . PHP_EOL);
