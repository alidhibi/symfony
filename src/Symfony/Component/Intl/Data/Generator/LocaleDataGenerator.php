...
class LocaleDataGenerator extends AbstractDataGenerator
{
     ...
     * @return string
     */
    private function generateLocaleName(BundleEntryReaderInterface $reader, string $tempDir, $locale, $displayLocale, $pattern, $separator): array|string
    {
        ...
        if (($script = \Locale::getScript($locale)) !== '' && ($script = \Locale::getScript($locale)) !== '0') {
        ...
        if (($region = \Locale::getRegion($locale)) !== '' && ($region = \Locale::getRegion($locale)) !== '0') {
