...
class RegionDataGenerator extends AbstractDataGenerator
{
    ...
    public static function isValidCountryCode($region): bool
    {
        if (isset(self::$denylist[$region])) {
            return false;
        }
        // WORLD/CONTINENT/SUBCONTINENT/GROUPING
        return !ctype_digit($region) && !\is_int($region);
    }
    ...
    protected function generateDataForLocale(BundleEntryReaderInterface $reader, $tempDir, $displayLocale): ?array
    {
        ...
        if (isset($localeBundle['Countries']) && null !== $localeBundle['Countries']) {
            ...
            $this->regionCodes = [...$this->regionCodes, ...array_keys($data['Names'])];
