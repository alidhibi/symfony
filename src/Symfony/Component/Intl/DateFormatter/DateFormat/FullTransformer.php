...
class FullTransformer
{
    ...
    public function __construct($pattern, $timezone)
    {
        ...
        $this->regExp = sprintf('/(%s|%s|%s)/', $this->quoteMatch, $implementedCharsMatch, $notImplementedCharsMatch);
     ...
     */
    public function formatReplace($dateChars, \DateTime $dateTime)
    {
     ...
     */
    public function getReverseMatchingRegExp($pattern): ?string
    {
        ...
        return preg_replace_callback($this->regExp, function (array $matches) {
            ...
            if (isset($transformers[$transformerIndex])) {
                ...
                return sprintf('(?P<%s>', $captureName).$transformer->getReverseMatchingRegExp($length).')';
            }
     ...
     */
    public function isQuoteMatch($quoteMatch): bool
    {
        return "'" === $quoteMatch[0];
    }
     ...
     */
    public function replaceQuoteMatch($quoteMatch): string
    {
     ...
     */
    protected function buildCharsMatch($specialChars): string
    {
        $specialCharsArray = str_split($specialChars);

        return implode('|', array_map(static fn($char): string => $char.'+', $specialCharsArray));
    }
