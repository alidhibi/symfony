...
class NumberFormatter
{
    ...
    final const PATTERN_DECIMAL = 0;

    final const DECIMAL = 1;

    final const CURRENCY = 2;

    final const PERCENT = 3;

    final const SCIENTIFIC = 4;

    final const SPELLOUT = 5;

    final const ORDINAL = 6;

    final const DURATION = 7;

    final const PATTERN_RULEBASED = 9;

    final const IGNORE = 0;

    final const DEFAULT_STYLE = 1;

    /* Format type constants */
    final const TYPE_DEFAULT = 0;

    final const TYPE_INT32 = 1;

    final const TYPE_INT64 = 2;

    final const TYPE_DOUBLE = 3;

    final const TYPE_CURRENCY = 4;

    /* Numeric attribute constants */
    final const PARSE_INT_ONLY = 0;

    final const GROUPING_USED = 1;

    final const DECIMAL_ALWAYS_SHOWN = 2;

    final const MAX_INTEGER_DIGITS = 3;

    final const MIN_INTEGER_DIGITS = 4;

    final const INTEGER_DIGITS = 5;

    final const MAX_FRACTION_DIGITS = 6;

    final const MIN_FRACTION_DIGITS = 7;

    final const FRACTION_DIGITS = 8;

    final const MULTIPLIER = 9;

    final const GROUPING_SIZE = 10;

    final const ROUNDING_MODE = 11;

    final const ROUNDING_INCREMENT = 12;

    final const FORMAT_WIDTH = 13;

    final const PADDING_POSITION = 14;

    final const SECONDARY_GROUPING_SIZE = 15;

    final const SIGNIFICANT_DIGITS_USED = 16;

    final const MIN_SIGNIFICANT_DIGITS = 17;

    final const MAX_SIGNIFICANT_DIGITS = 18;

    final const LENIENT_PARSE = 19;

    /* Text attribute constants */
    final const POSITIVE_PREFIX = 0;

    final const POSITIVE_SUFFIX = 1;

    final const NEGATIVE_PREFIX = 2;

    final const NEGATIVE_SUFFIX = 3;

    final const PADDING_CHARACTER = 4;

    final const CURRENCY_CODE = 5;

    final const DEFAULT_RULESET = 6;

    final const PUBLIC_RULESETS = 7;

    /* Format symbol constants */
    final const DECIMAL_SEPARATOR_SYMBOL = 0;

    final const GROUPING_SEPARATOR_SYMBOL = 1;

    final const PATTERN_SEPARATOR_SYMBOL = 2;

    final const PERCENT_SYMBOL = 3;

    final const ZERO_DIGIT_SYMBOL = 4;

    final const DIGIT_SYMBOL = 5;

    final const MINUS_SIGN_SYMBOL = 6;

    final const PLUS_SIGN_SYMBOL = 7;

    final const CURRENCY_SYMBOL = 8;

    final const INTL_CURRENCY_SYMBOL = 9;

    final const MONETARY_SEPARATOR_SYMBOL = 10;

    final const EXPONENTIAL_SYMBOL = 11;

    final const PERMILL_SYMBOL = 12;

    final const PAD_ESCAPE_SYMBOL = 13;

    final const INFINITY_SYMBOL = 14;

    final const NAN_SYMBOL = 15;

    final const SIGNIFICANT_DIGIT_SYMBOL = 16;

    final const MONETARY_GROUPING_SEPARATOR_SYMBOL = 17;

    /* Rounding mode values used by NumberFormatter::setAttribute() with NumberFormatter::ROUNDING_MODE attribute */
    final const ROUND_CEILING = 0;

    final const ROUND_FLOOR = 1;

    final const ROUND_DOWN = 2;

    final const ROUND_UP = 3;

    final const ROUND_HALFEVEN = 4;

    final const ROUND_HALFDOWN = 5;

    final const ROUND_HALFUP = 6;

    /* Pad position values used by NumberFormatter::setAttribute() with NumberFormatter::PADDING_POSITION attribute */
    final const PAD_BEFORE_PREFIX = 0;

    final const PAD_AFTER_PREFIX = 1;

    final const PAD_BEFORE_SUFFIX = 2;

    final const PAD_AFTER_SUFFIX = 3;
     ...
     */
    private static array $supportedStyles = [
     ...
     */
    private static array $supportedAttributes = [
     ...
     */
    private static array $phpRoundingMap = [
     ...
     */
    private static array $customRoundingList = [
     ...
     */
    private static int $int32Max = 2_147_483_647;

    /**
     * The maximum value of the integer type in 64 bit platforms.
     *
     */
    private static int $int64Max = 9_223_372_036_854_775_807;

    private static array $enSymbols = [
        self::DECIMAL => ['.', ',', ';', '%', '0', '#', '-', '+', '¤', '¤¤', '.', 'E', '‰', '*', '∞', 'NaN', '@', ','],
        self::CURRENCY => ['.', ',', ';', '%', '0', '#', '-', '+', '¤', '¤¤', '.', 'E', '‰', '*', '∞', 'NaN', '@', ','],
    ];

    private static array $enTextAttributes = [
        self::DECIMAL => ['', '', '-', '', ' ', 'XXX', ''],
        self::CURRENCY => ['¤', '', '-¤', '', ' ', 'XXX'],
    ];
     ...
     * @throws MethodArgumentNotImplementedException      When the pattern value is different than null
     */
    public static function create($locale = 'en', $style = null, $pattern = null): self
    {
     ...
     */
    public function getPattern(): never
    {
     ...
     * @throws MethodNotImplementedException
     */
    public function parseCurrency($value, &$currency, &$position = null): never
    {
        throw new MethodNotImplementedException(__METHOD__);
    }
    ...
    public function parse($value, $type = self::TYPE_DOUBLE, &$position = 0)
    {
        ...
        if (preg_match(sprintf('/^-?(?:\.\d++%s|\d++(\.\d*+)?)/', $groupingMatch), $value, $matches)) {
            $value = $matches[0];
     ...
     * @throws MethodNotImplementedException
     */
    public function setPattern($pattern): never
    {
        throw new MethodNotImplementedException(__METHOD__);
    }
     ...
     * @throws MethodNotImplementedException
     */
    public function setSymbol($attr, $value): never
    {
        throw new MethodNotImplementedException(__METHOD__);
    }
     ...
     * @throws MethodNotImplementedException
     */
    public function setTextAttribute($attr, $value): never
    {
        throw new MethodNotImplementedException(__METHOD__);
    }
     ...
     */
    private function formatNumber($value, $precision): string
    {
        $precision = $this->getUninitializedPrecision($value, $precision);
     ...
     */
    private function isInitializedAttribute(int $attr): bool
    {
     ...
     */
    private function convertValueDataType(array|string $value, $type)
    {
        $type = (int) $type;
     ...
     */
    private function getInt32Value(array|string $value): bool|int
    {
     ...
     * @return int|float|false The converted value
     */
    private function getInt64Value(array|string $value): bool|float|int
    {
     ...
     */
    private function isInvalidRoundingMode($value): bool
    {
