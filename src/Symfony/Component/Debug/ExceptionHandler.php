...
class ExceptionHandler
{
    ...
    private int|bool|null $caughtLength = null;
     ...
     *
     */
    public static function register($debug = true, $charset = null, $fileLinkFormat = null): static
    {
        $handler = new static($debug, $charset, $fileLinkFormat);

        $prev = set_exception_handler(static fn(\Exception $exception) => $handler->handle($exception));
        if (\is_array($prev) && $prev[0] instanceof ErrorHandler) {
            restore_exception_handler();
            $prev[0]->setExceptionHandler(static fn(\Exception $exception) => $handler->handle($exception));
        }
...
EOF;
    ...
    private function decorate($content, $css): string
    {
        return <<<EOF
<!DOCTYPE html>
<html>
...
EOF;
    ...
    private function formatPath($path, ?string $line): string
    {
     ...
     */
    private function escapeHtml($str): string
    {
