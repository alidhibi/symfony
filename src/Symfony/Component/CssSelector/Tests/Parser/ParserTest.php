...
class ParserTest extends TestCase
{
    ...
    public function testParser(string $source, array $representation): void
    {
        $parser = new Parser();

        $this->assertEquals($representation, array_map(static fn(SelectorNode $node): string => (string) $node->getTree(), $parser->parse($source)));
    }

    /** @dataProvider getParserExceptionTestData */
    public function testParserException(string $source, string $message): void
    {
        $parser = new Parser();
        ...
        try {
            ...
            $this->assertEquals($message, $syntaxErrorException->getMessage());
        }
    }

    /** @dataProvider getPseudoElementsTestData */
    public function testPseudoElements(string $source, string $element, string $pseudo): void
    {
        $parser = new Parser();
    ...
    public function testSpecificity(string $source, int $value): void
    {
        $parser = new Parser();
        $selectors = $parser->parse($source);
        $this->assertCount(1, $selectors);
    ...
    public function testParseSeries(string $series, int $a, int $b): void
    {
        $parser = new Parser();
    ...
    public function testParseSeriesException(string $series): void
    {
        $parser = new Parser();
        $selectors = $parser->parse(sprintf(':nth-child(%s)', $series));
        $this->assertCount(1, $selectors);
