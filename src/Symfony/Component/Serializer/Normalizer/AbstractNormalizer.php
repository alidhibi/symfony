...
abstract class AbstractNormalizer extends SerializerAwareNormalizer implements NormalizerInterface, DenormalizerInterface, SerializerAwareInterface
{
    ...
    final const CIRCULAR_REFERENCE_LIMIT = 'circular_reference_limit';

    final const OBJECT_TO_POPULATE = 'object_to_populate';

    final const GROUPS = 'groups';

    final const ATTRIBUTES = 'attributes';

    final const ALLOW_EXTRA_ATTRIBUTES = 'allow_extra_attributes';
     ...
     */
    protected function isCircularReference($object, array &$context)
    {
            ...
            foreach ($constructorParameters as $constructorParameter) {
                ...
                if (method_exists($constructorParameter, 'isVariadic') && $constructorParameter->isVariadic()) {
                    if ($allowed && !$ignored && (isset($data[$key]) || \array_key_exists($key, $data))) {
                        ...
                        $params = [...$params, ...$variadicParameters];
                        unset($data[$key]);
                    }
    ...
    protected function denormalizeParameter(\ReflectionClass $class, \ReflectionParameter $parameter, $parameterName, $parameterData, array $context, $format = null)
    {
        try {
            ...
            throw new RuntimeException(sprintf('Could not determine the class of the parameter "%s".', $parameterName), 0, $reflectionException);
        }
    }
