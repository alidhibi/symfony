...
class XmlEncoder extends SerializerAwareEncoder implements EncoderInterface, DecoderInterface, NormalizationAwareInterface
{
     ...
     */
    public function __construct(string $rootNodeName = 'response', $loadOptions = null)
    {
     ...
     * {@inheritdoc}
     */
    public function encode($data, $format, array $context = []): string|bool
    {
     ...
     *
     */
    final protected function appendXMLString(\DOMNode $node, $val): bool
    {
        if (\strlen($val) > 0) {
     ...
     *
     */
    final protected function appendText(\DOMNode $node, $val): bool
    {
     ...
     *
     */
    final protected function appendCData(\DOMNode $node, $val): bool
    {
     ...
     *
     */
    final protected function isElementNameValid($name): bool
    {
        return $name &&
            false === strpos($name, ' ') &&
    ...
    private function parseXmlValue(\DOMNode $node, array $context = [])
    {
        ...
        foreach ($node->childNodes as $subnode) {
            ...
            if ('item' === $subnode->nodeName && isset($val['@key'])) {
                $value[$val['@key']] = isset($val['#']) ? $val['#'] : $val;
            } else {
    ...
    private function buildXml(\DOMNode $parentNode, $data, $xmlRootNodeName = null)
    {
        ...
        throw new NotEncodableValueException('An unexpected value could not be serialized: '.(\is_resource($data) ? sprintf('%s resource', get_resource_type($data)) : var_export($data, true)));
    }
