...
{
    protected \Symfony\Component\BrowserKit\History $history;

    protected \Symfony\Component\BrowserKit\CookieJar $cookieJar;
    ...
    {
        $this->server = ['HTTP_USER_AGENT' => 'Symfony BrowserKit', ...$server];
    }
    ...
    public function request($method, $uri, array $parameters = [], array $files = [], array $server = [], $content = null, $changeHistory = true)
    {
        ...
        $this->response = $this->insulated ? $this->doRequestInProcess($this->request) : $this->doRequest($this->request);
        ...
        $this->redirect = $status >= 300 && $status < 400 ? $this->internalResponse->getHeader('Location') : null;
    ...
    public function followRedirect()
    {
        ...
        if (-1 !== $this->maxRedirects && $this->redirectCount > $this->maxRedirects) {
            $this->redirectCount = 0;
        ...
        $parameters = 'GET' === strtoupper($method) ? [] : $request->getParameters();
    ...
    private function updateServerFromUri(array $server, $uri)
    {
        $server['HTTP_HOST'] = $this->extractHost($uri);
    ...
    private function extractHost($uri)
    {
        ...
        if (($port = parse_url($uri, \PHP_URL_PORT)) !== null) {
            return $host.':'.$port;
        }
