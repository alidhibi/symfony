...
{
    private ?\Symfony\Component\Yaml\Dumper $dumper = null;
    ...
    public function dump(array $options = [])
    {
        ...
        if (!$this->dumper instanceof \Symfony\Component\Yaml\Dumper) {
     ...
     *
     */
    private function addService($id, Definition $definition): string
    {
        $code = "    {$id}:\n";
        ...
        if ($autowiringTypesCode !== '' && $autowiringTypesCode !== '0') {
     ...
     *
     */
    private function addServiceAlias($alias, Alias $id): string
    {
        if ($id->isPrivate()) {
            return sprintf("    %s: '@%s'\n", $alias, $id);
        }
    ...
    {
        if ($reference instanceof \Symfony\Component\DependencyInjection\Reference) {
