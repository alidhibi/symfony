...
class PhpDumper extends Dumper
{
     ...
     */
    final const FIRST_CHARS = 'abcdefghijklmnopqrstuvwxyz';

    /**
     * Characters that might appear in the generated variable name as any but the first character.
     */
    final const NON_FIRST_CHARS = 'abcdefghijklmnopqrstuvwxyz0123456789_';

    private \SplObjectStorage|null|array $definitionVariables = null;

    private ?array $referenceVariables = null;

    private ?int $variableCount = null;

    private $inlinedDefinitions;

    private ?array $serviceCalls = null;

    private array $reservedVariables = ['instance', 'class', 'this'];

    private ?\Symfony\Component\DependencyInjection\ExpressionLanguage $expressionLanguage = null;

    private ?string $targetDirRegex = null;

    private ?int $targetDirMaxMatches = null;

    private ?string $docStar = null;

    private ?array $serviceIdToMethodNameMap = null;

    private ?array $usedMethodNames = null;
    ...
    private ?bool $inlineRequires = null;

    private $inlinedRequires = [];

    private array $circularReferences = [];

    private \Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\DumperInterface|\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\NullDumper|null $proxyDumper = null;
    ...
    public function dump(array $options = [])
    {
        ...
        $options = ['class' => 'ProjectServiceContainer', 'base_class' => 'Container', 'namespace' => '', 'as_files' => false, 'debug' => true, 'hot_path_tag' => 'container.hot_path', 'inline_class_loader_parameter' => 'container.dumper.inline_class_loader', 'build_time' => time(), ...$options];
...
EOF;
            ...
            foreach ($files as $file => $c) {
                $code[sprintf('Container%s/%s', $hash, $file)] = $c;
            }

            array_pop($code);
            $code[sprintf('Container%s/%s.php', $hash, $options['class'])] = substr_replace($files[$options['class'].'.php'], "<?php\n\nnamespace Container{$hash};\n", 0, 6);
...
EOF;
     ...
     * @return ProxyDumper
     */
    private function getProxyDumper(): ?\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\DumperInterface
    {
        if (!$this->proxyDumper instanceof \Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\DumperInterface) {
    ...
    private function analyzeCircularReferences($sourceId, array $edges, array &$checkedNodes, array &$currentPath = [], $byConstructor = true): void
    {
    ...
    private function connectCircularReferences($sourceId, array &$currentPath, $byConstructor, array &$subPath = []): void
    {
    ...
    private function addCircularReferences($id, array $currentPath, $byConstructor): void
    {
    ...
    private function generateProxyClasses()
    {
        ...
        $strip = '' === $this->docStar && method_exists(\Symfony\Component\HttpKernel\Kernel::class, 'stripComments');
     ...
     *
     */
    private function addServiceInclude($cId, Definition $definition): string
    {
        $code = '';
        ...
        if ($this->inlineRequires && !$this->isHotPath($definition)) {
            ...
            foreach (array_keys(array_diff_key(array_flip($lineage), $this->inlinedRequires)) as $file) {
     ...
     */
    private function addServiceInstance($id, Definition $definition, bool $isSimpleInstance)
    {
     ...
     *
     */
    private function addServiceMethodCalls(Definition $definition, $variableName = 'instance'): string
    {
        $calls = '';
    ...
    private function addServiceProperties(Definition $definition, $variableName = 'instance'): string
    {
        $code = '';
     ...
     *
     */
    private function addServiceConfigurator(Definition $definition, $variableName = 'instance'): string
    {
     ...
     *
     */
    private function addService($id, Definition $definition, &$file = null): string
    {
        ...
        $lazyInitialization = $definition->isLazy() ? '$lazyLoad = true' : '';
...
EOF;
        ...
        if ($asFile) {
            $code = implode("\n", array_map(static fn($line): string|array|int|float|false|null => $line !== '' && $line !== '0' ? substr($line, 8) : $line, explode("\n", $code)));
        } else {
            $code .= "    }\n";
        }
        $this->definitionVariables = null;
        $this->inlinedDefinitions = null;
    ...
    private function addInlineVariables($id, Definition $definition, array $arguments, $forConstructor): string
    {
        $code = '';
...
EOTXT
    ...
    private function addInlineService($id, Definition $definition, Definition $inlineDef = null, $forConstructor = true): string
    {
        $code = '';

        if ($isSimpleInstance = $isRootInstance = !$inlineDef instanceof \Symfony\Component\DependencyInjection\Definition) {
    ...
    private function addNewInstance(Definition $definition, string $return, string $instantiation, $id): string
    {
        $class = $this->dumpValue($definition->getClass());
     ...
     *
     */
    private function startClass($class, $baseClass, $baseClassWithNamespace): string
    {
...
EOF;
     ...
     *
     */
    private function addNormalizedIds(): string
    {
        $code = '';
        ...
        return $code !== '' && $code !== '0' ? "        \$this->normalizedIds = [\n".$code."        ];\n" : '';
    }
    ...
    private function addSyntheticIds(): string
    {
        ...
        return $code !== '' && $code !== '0' ? "        \$this->syntheticIds = [\n{$code}        ];\n" : '';
    }
...
EOF;
    ...
    private function addMethodMap(): string
    {
        ...
        return $code !== '' && $code !== '0' ? "        \$this->methodMap = [\n{$code}        ];\n" : '';
    }
    ...
    private function addFileMap(): string
    {
        ...
        return $code !== '' && $code !== '0' ? "        \$this->fileMap = [\n{$code}        ];\n" : '';
    }
     ...
     *
     */
    private function addPrivateServices(): string
    {
        $code = '';
...
EOF;
     ...
     * @throws InvalidArgumentException
     */
    private function exportParameters(array $parameters, string $path = '', int|float $indent = 12): string
    {
        $php = [];
...
EOF;
    ...
    private function wrapServiceConditionals($value, string $code): string
    {
        ...
        $code = implode("\n", array_map(static fn($line): string|array|int|float|false|null => $line !== '' && $line !== '0' ? '    '.$line : $line, explode("\n", $code)));
    ...
    {
        if (!$definitions instanceof \SplObjectStorage) {
    ...
    private function dumpValue($value, $interpolate = true)
    {
        if (\is_array($value)) {
            if ($value && $interpolate && false !== $param = array_search($value, $this->container->getParameterBag()->all(), true)) {
                return $this->dumpValue(sprintf('%%%s%%', $param));
            }
                ...
                if ($value instanceof IteratorArgument) {
                    ...
                    if (!$values = $value->getValues()) {
                        ...
                        foreach ($values as $k => $v) {
                            ($c = $this->getServiceConditionals($v)) ? $operands[] = sprintf('(int) (%s)', $c) : ++$operands[0];
            ...
            if (null !== $value->getFactory()) {
                ...
                if (\is_array($factory)) {
                    ...
                    if ($factory[0] instanceof Definition) {
                        ...
                        return sprintf("\\call_user_func([%s, '%s']%s)", $class, $factory[1], $arguments !== [] ? ', '.implode(', ', $arguments) : '');
                    }
     ...
     *
     */
    private function getServiceCall($id, ?\Symfony\Component\DependencyInjection\Reference $reference = null): string
    {
        while ($this->container->hasAlias($id)) {
            $id = (string) $this->container->getAlias($id);
        }
        ...
        if ($this->container->hasDefinition($id) && $definition = $this->container->getDefinition($id)) {
            if ($definition->isSynthetic()) {
                $code = sprintf('$this->get(%s%s)', $this->doExport($id), $reference instanceof \Symfony\Component\DependencyInjection\Reference ? ', '.$reference->getInvalidBehavior() : '');
            } elseif ($reference instanceof \Symfony\Component\DependencyInjection\Reference && ContainerInterface::IGNORE_ON_UNINITIALIZED_REFERENCE === $reference->getInvalidBehavior()) {
                $code = 'null';
            ...
            return 'null';
        } elseif ($reference instanceof \Symfony\Component\DependencyInjection\Reference && ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE !== $reference->getInvalidBehavior()) {
     ...
     */
    private function initializeMethodNamesMap($class): void
    {
    ...
    private function getExpressionLanguage(): ?\Symfony\Component\DependencyInjection\ExpressionLanguage
    {
        if (!$this->expressionLanguage instanceof \Symfony\Component\DependencyInjection\ExpressionLanguage) {
    ...
    private function export($value)
    {
        if (null !== $this->targetDirRegex && \is_string($value) && preg_match($this->targetDirRegex, $value, $matches, \PREG_OFFSET_CAPTURE)) {
            ...
            $prefix = $matches[0][1] !== 0 ? $this->doExport(substr($value, 0, $matches[0][1]), true).'.' : '';
