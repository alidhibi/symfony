...
{
    final const NS = 'http://symfony.com/schema/dic/services';
    ...
    private function getServiceDefaults(\DOMDocument $xml, $file): array
    {
        ...
        $defaults = [
            ...
            'bind' => array_map(static fn($v): \Symfony\Component\DependencyInjection\Argument\BoundArgument => new BoundArgument($v), $this->getArgumentsAsPhp($defaultsNode, 'bind', $file)),
        ];
     ...
     *
     */
    private function parseDefinition(\DOMElement $service, $file, array $defaults): ?\Symfony\Component\DependencyInjection\Definition
    {
        if (($alias = $service->getAttribute('alias')) !== '' && ($alias = $service->getAttribute('alias')) !== '0') {
            $this->validateAlias($service, $file);

            $this->container->setAlias((string) $service->getAttribute('id'), $alias = new Alias($alias));
            if (($publicAttr = $service->getAttribute('public')) !== '' && ($publicAttr = $service->getAttribute('public')) !== '0') {
        ...
        } elseif (($parent = $service->getAttribute('parent')) !== '' && ($parent = $service->getAttribute('parent')) !== '0') {
        ...
        foreach (['class', 'public', 'shared', 'synthetic', 'lazy', 'abstract'] as $key) {
            if (($value = $service->getAttribute($key)) !== '' && ($value = $service->getAttribute($key)) !== '0') {
                $method = 'set'.$key;
                $definition->$method(XmlUtils::phpize($value));
            }
        }

        if (($value = $service->getAttribute('autowire')) !== '' && ($value = $service->getAttribute('autowire')) !== '0') {
            $definition->setAutowired(XmlUtils::phpize($value));
        }

        if (($value = $service->getAttribute('autoconfigure')) !== '' && ($value = $service->getAttribute('autoconfigure')) !== '0') {
        ...
        if ($factories = $this->getChildren($service, 'factory')) {
            $factory = $factories[0];
            if (($function = $factory->getAttribute('function')) !== '' && ($function = $factory->getAttribute('function')) !== '0') {
                $definition->setFactory($function);
            } else {
                if (($childService = $factory->getAttribute('service')) !== '' && ($childService = $factory->getAttribute('service')) !== '0') {
        ...
        if ($configurators = $this->getChildren($service, 'configurator')) {
            ...
            if (($function = $configurator->getAttribute('function')) !== '' && ($function = $configurator->getAttribute('function')) !== '0') {
                $definition->setConfigurator($function);
            } else {
                if (($childService = $configurator->getAttribute('service')) !== '' && ($childService = $configurator->getAttribute('service')) !== '0') {
                    $class = new Reference($childService, ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE);
                } else {
        ...
        if (($value = $service->getAttribute('decorates')) !== '' && ($value = $service->getAttribute('decorates')) !== '0') {
    ...
    {
        try {
            $dom = XmlUtils::loadFile($file, fn(\DOMDocument $dom): bool => $this->validateSchema($dom));
        } catch (\InvalidArgumentException $invalidArgumentException) {
            throw new InvalidArgumentException(sprintf('Unable to parse file "%s": ', $file).$invalidArgumentException->getMessage(), $invalidArgumentException->getCode(), $invalidArgumentException);
        }
     ...
     * @return mixed[]
     */
    private function getArgumentsAsPhp(\DOMElement $node, $name, $file, bool $isChildDefinition = false): array
    {
        $arguments = [];
        foreach ($this->getChildren($node, $name) as $arg) {
            ...
            switch ($arg->getAttribute('type')) {
                ...
                case 'iterator':
                    ...
                    break;
                case 'tagged':
                    if ($arg->getAttribute('tag') === '' || $arg->getAttribute('tag') === '0') {
    ...
    public function validateSchema(\DOMDocument $dom)
    {
        ...
        if (($element = $dom->documentElement->getAttributeNS('http://www.w3.org/2001/XMLSchema-instance', 'schemaLocation')) !== '' && ($element = $dom->documentElement->getAttributeNS('http://www.w3.org/2001/XMLSchema-instance', 'schemaLocation')) !== '0') {
    ...
    private function validateExtensions(\DOMDocument $dom, $file): void
    {
        foreach ($dom->documentElement->childNodes as $node) {
            ...
            if (!$this->container->hasExtension($node->namespaceURI)) {
                $extensionNamespaces = array_filter(array_map(static fn($ext) => $ext->getNamespace(), $this->container->getExtensions()));
