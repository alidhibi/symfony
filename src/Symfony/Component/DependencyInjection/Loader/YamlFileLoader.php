...
{
    private static array $serviceKeywords = [
        'alias' => 'alias',
        'parent' => 'parent',
        'class' => 'class',
        'shared' => 'shared',
    ...
    private static array $prototypeKeywords = [
        'resource' => 'resource',
        'namespace' => 'namespace',
        'exclude' => 'exclude',
        'parent' => 'parent',
    ...
    private static array $instanceofKeywords = [
        'shared' => 'shared',
        'lazy' => 'lazy',
        'public' => 'public',
        'properties' => 'properties',
    ...
    private static array $defaultsKeywords = [
        'public' => 'public',
        'tags' => 'tags',
        'autowire' => 'autowire',
        'autoconfigure' => 'autoconfigure',
    ...
    private ?int $anonymousServicesCount = null;

    private ?string $anonymousServicesSuffix = null;
    ...
    private function parseImports(array $content, $file): void
    {
        ...
        foreach ($content['imports'] as $import) {
            ...
            $this->import($import['resource'], isset($import['type']) ? $import['type'] : null, isset($import['ignore_errors']) && (bool) $import['ignore_errors'], $file);
        }
    }
     ...
     * @param string $file
     */
    private function parseDefinitions(array $content, $file): void
    {
    ...
    private function parseDefaults(array &$content, $file)
    {
        ...
        if (isset($defaults['bind'])) {
            ...
            $defaults['bind'] = array_map(static fn($v): \Symfony\Component\DependencyInjection\Argument\BoundArgument => new BoundArgument($v), $this->resolveServices($defaults['bind'], $file));
        }
     ...
     */
    private function parseCallable($callable, string $parameter, $id, $file)
    {
    ...
    protected function loadFile($file)
    {
        ...
        set_error_handler(static function ($level, $message, $script, $line) use ($file, &$prevErrorHandler) : bool {
        ...
        try {
            ...
            throw new InvalidArgumentException(sprintf('The file "%s" does not contain valid YAML: ', $file).$parseException->getMessage(), 0, $parseException);
        } finally {
    ...
    private function validate($content, $file): ?array
    {
        ...
        foreach (array_keys($content) as $namespace) {
            ...
            if (!$this->container->hasExtension($namespace)) {
                $extensionNamespaces = array_filter(array_map(static fn($ext) => $ext->getAlias(), $this->container->getExtensions()));
