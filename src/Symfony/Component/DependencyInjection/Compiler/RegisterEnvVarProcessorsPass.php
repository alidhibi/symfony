...
class RegisterEnvVarProcessorsPass implements CompilerPassInterface
{
    ...
    public function process(ContainerBuilder $container): void
    {
        ...
        $types = [];
        $processors = [];
        foreach (array_keys($container->findTaggedServiceIds('container.env_var_processor')) as $id) {
            if (($r = $container->getReflectionClass($class = $container->getDefinition($id)->getClass())) === null) {
            ...
            foreach ($class::getProvidedTypes() as $prefix => $type) {
                ...
                $types[$prefix] = $this->validateProvidedTypes($type, $class);
            }
        }
        ...
        if ($bag instanceof EnvPlaceholderParameterBag) {
            foreach (EnvVarProcessor::getProvidedTypes() as $prefix => $type) {
                if (!isset($types[$prefix])) {
                    $types[$prefix] = $this->validateProvidedTypes($type, EnvVarProcessor::class);
                }
            }
     ...
     * @return string[]
     */
    private function validateProvidedTypes($types, $class): array
    {
        $types = explode('|', $types);
