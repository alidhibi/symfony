...
class AutowirePass extends AbstractRecursivePass
{
    ...
    private ?array $ambiguousServiceTypes = null;
     ...
     */
    public static function createResourceForClass(\ReflectionClass $reflectionClass): \Symfony\Component\DependencyInjection\Config\AutowireServiceResource
    {
    ...
    protected function processValue($value, $isRoot = false)
    {
        try {
            ...
            if ($this->throwOnAutowiringException) {
                throw $autowiringFailedException;
            }

            $this->autowiringExceptions[] = $autowiringFailedException;
            $this->container->getDefinition($this->currentId)->addError($autowiringFailedException->getMessage());
    ...
    private function doProcessValue($value, $isRoot = false)
    {
        if ($value instanceof TypedReference) {
            if (($ref = $this->getAutowiredReference($value, $value->getRequiringClass() ? sprintf('for "%s" in "%s"', $value->getType(), $value->getRequiringClass()) : '')) !== null) {
                return $ref;
            }
        ...
        if (($reflectionClass = $this->container->getReflectionClass($value->getClass(), false)) === null) {
        ...
        try {
            ...
            throw new AutowiringFailedException($this->currentId, $runtimeException->getMessage(), 0, $runtimeException);
        }
    ...
    private function autowireMethod(\ReflectionFunctionAbstract $reflectionMethod, array $arguments): array
    {
        ...
        foreach ($parameters as $index => $parameter) {
            ...
            if (($value = $this->getAutowiredReference($ref = new TypedReference($type, $type, $parameter->isOptional() ? '' : $class), 'for '.sprintf('argument "$%s" of method "%s()"', $parameter->name, $class.'::'.$method))) === null) {
     ...
     */
    private function getAutowiredReference(TypedReference $reference, string $deprecationMessage)
    {
     ...
     */
    private function populateAvailableTypes(?bool $onlyAutowiringTypes = false): void
    {
        $this->types = [];
     ...
     * @param string $id
     */
    private function populateAvailableType($id, Definition $definition, $onlyAutowiringTypes): void
    {
     ...
     * @param string $id
     */
    private function set($type, $id): void
    {
        if (isset($this->definedTypes[$type])) {
            return;
        }
     ...
     */
    private function createAutowiredDefinition(string $type): ?\Symfony\Component\DependencyInjection\TypedReference
    {
        ...
        try {
            ...
            $this->lastFailure = $autowiringFailedException->getMessage();
    ...
    private function createTypeNotFoundMessage(TypedReference $reference, string $label): string
    {
        $trackResources = $this->container->isTrackingResources();
        $this->container->setResourceTracking(false);
        try {
            if (($r = $this->container->getReflectionClass($type = $reference->getType(), false)) !== null) {
    ...
    private function createTypeAlternatives(TypedReference $reference): string
    {
     ...
     * @return array<mixed, array<'class'|'defaultValue'|'isOptional', mixed>>
     */
    private static function getResourceMetadataForMethod(\ReflectionMethod $method): array
    {
    ...
    private function getAliasesSuggestionForType($type, $extraContext = null): ?string
    {
        $aliases = [];
