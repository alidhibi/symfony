...
class ContainerBuilder extends Container implements TaggedContainerInterface
{
    ...
    private ?\Symfony\Component\DependencyInjection\Compiler\Compiler $compiler = null;

    private bool $trackResources;

    private ?\Symfony\Component\DependencyInjection\LazyProxy\Instantiator\InstantiatorInterface $proxyInstantiator = null;
    ...
    private static array $internalTypes = [
        'int' => true,
        'float' => true,
        'string' => true,
        'bool' => true,
    ...
    public function __construct(ParameterBagInterface $parameterBag = null)
    {
        ...
        $this->trackResources = interface_exists(\Symfony\Component\Config\Resource\ResourceInterface::class);
    ...
    {
        if (!$this->compiler instanceof \Symfony\Component\DependencyInjection\Compiler\Compiler) {
    ...
    private function doGet($id, $invalidBehavior = ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE, array &$inlineServices = null, $isConstructorArgument = false)
    {
        ...
        try {
            ...
            if (($service = parent::get($id, ContainerInterface::NULL_ON_INVALID_REFERENCE)) !== null) {
                return $service;
            }
        } catch (ServiceCircularReferenceException $serviceCircularReferenceException) {
            if ($isConstructorArgument) {
                throw $serviceCircularReferenceException;
            }
        }
     ...
     */
    public function compile(/*$resolveEnvPlaceholders = false*/): void
    {
        if (1 <= \func_num_args()) {
     ...
     */
    private function createService(Definition $definition, array &$inlineServices, $isConstructorArgument = false, $id = null, bool $tryProxy = true)
    {
    ...
    public function findTags()
    {
        ...
        foreach ($this->getDefinitions() as $definition) {
            $tags = [...array_keys($definition->getTags()), ...$tags];
        }
        ...
        foreach ($envPlaceholders as $env => $placeholders) {
            foreach ($placeholders as $placeholder) {
                if (false !== stripos($value, $placeholder)) {
                    $resolved = true === $format ? $bag->escapeValue($this->getEnv($env)) : sprintf($format, $env);
    ...
    public function getEnvCounters()
    {
        ...
        foreach (array_keys($envPlaceholders) as $env) {
            if (!isset($this->envCounters[$env])) {
     ...
     * @internal
     */
    public function removeBindings($id): void
    {
    ...
    protected function getEnv($name)
    {
        ...
        foreach ($bag->getEnvPlaceholders() as $placeholders) {
            if (isset($placeholders[$value])) {
                ...
                return $bag->unescapeValue($bag->get(sprintf('env(%s)', $name)));
            }
        }

        $this->resolving[sprintf('env(%s)', $name)] = true;
        try {
            return $bag->unescapeValue($this->resolveEnvPlaceholders($bag->escapeValue($value), true));
        } finally {
            unset($this->resolving[sprintf('env(%s)', $name)]);
        }
    }

    private function callMethod($service, array $call, array &$inlineServices): void
    {
     ...
     */
    private function shareService(Definition $definition, $service, $id, array &$inlineServices): void
    {
