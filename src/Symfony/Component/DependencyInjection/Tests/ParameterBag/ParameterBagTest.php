...
class ParameterBagTest extends TestCase
{
    ...
    public function testGetSet(): void
    {
        ...
        try {
            ...
            $this->assertInstanceOf(\Symfony\Component\DependencyInjection\Exception\ParameterNotFoundException::class, $exception, '->get() throws an Symfony\Component\DependencyInjection\Exception\ParameterNotFoundException if the key does not exist');
            $this->assertEquals('You have requested a non-existent parameter "baba".', $exception->getMessage(), '->get() throws an Symfony\Component\DependencyInjection\Exception\ParameterNotFoundException if the key does not exist');
        }
    }
     ...
     */
    public function testGetThrowParameterNotFoundException(string $parameterKey, string $exceptionMessage): void
    {
        $bag = new ParameterBag([
            'foo' => 'foo',
            'bar' => 'bar',
    ...
    public function provideGetThrowParameterNotFoundExceptionData(): array
    {
        return [
    ...
    public function testResolveValue(): void
    {
        ...
        try {
            ...
            $this->assertEquals('You have requested a non-existent parameter "foobar".', $parameterNotFoundException->getMessage(), '->resolveValue() throws a ParameterNotFoundException if a placeholder references a non-existent parameter');
        }
        ...
        try {
            ...
            $this->assertEquals('A string value must be composed of strings and/or numbers, but found parameter "bar" of type "array" inside string value "a %bar%".', $runtimeException->getMessage(), '->resolveValue() throws a RuntimeException when a parameter embeds another non-string parameter');
        }
        ...
        } catch (ParameterCircularReferenceException $parameterCircularReferenceException) {
            $this->assertEquals('Circular reference detected for parameter "foo" ("foo" > "bar" > "foobar" > "foo").', $parameterCircularReferenceException->getMessage(), '->resolveValue() throws a ParameterCircularReferenceException when a parameter has a circular reference');
        }
    ...
    public function testResolveIndicatesWhyAParameterIsNeeded(): void
    {
        ...
        try {
            ...
            $this->assertEquals('The parameter "foo" has a dependency on a non-existent parameter "bar".', $parameterNotFoundException->getMessage());
        }
     ...
     */
    public function testResolveStringWithSpacesReturnsString(string $expected, string $test, string $description): void
    {
        $bag = new ParameterBag(['foo' => 'bar']);
    ...
    public function stringsWithSpacesProvider(): array
    {
        return [
