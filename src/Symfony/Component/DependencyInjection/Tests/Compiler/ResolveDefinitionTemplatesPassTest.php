...
class ResolveDefinitionTemplatesPassTest extends TestCase
{
    ...
    public function testDeepDefinitionsResolving(): void
    {
        ...
        $this->assertInstanceOf(\Symfony\Component\DependencyInjection\Definition::class, $configurator);
        $this->assertSame('parentClass', $configurator->getClass());

        $factory = $container->getDefinition('sibling')->getFactory();
        $this->assertInstanceOf(\Symfony\Component\DependencyInjection\Definition::class, $factory[0]);
        $this->assertSame('parentClass', $factory[0]->getClass());

        $argument = $container->getDefinition('sibling')->getArgument(0);
        $this->assertInstanceOf(\Symfony\Component\DependencyInjection\Definition::class, $argument);
        $this->assertSame('parentClass', $argument->getClass());

        $properties = $container->getDefinition('sibling')->getProperties();
        $this->assertInstanceOf(\Symfony\Component\DependencyInjection\Definition::class, $properties['prop']);
        $this->assertSame('parentClass', $properties['prop']->getClass());

        $methodCalls = $container->getDefinition('sibling')->getMethodCalls();
        $this->assertInstanceOf(\Symfony\Component\DependencyInjection\Definition::class, $methodCalls[0][1][0]);
