...
class ResolveChildDefinitionsPassTest extends TestCase
{
    ...
    public function testDeepDefinitionsResolving(): void
    {
        ...
        $container->register('sibling', 'siblingClass')
            ->setConfigurator(new ChildDefinition('parent'))
        ...
        $this->assertSame(\Symfony\Component\DependencyInjection\Definition::class, \get_class($configurator));
        $this->assertSame('parentClass', $configurator->getClass());

        $factory = $container->getDefinition('sibling')->getFactory();
        $this->assertSame(\Symfony\Component\DependencyInjection\Definition::class, \get_class($factory[0]));
        $this->assertSame('parentClass', $factory[0]->getClass());

        $argument = $container->getDefinition('sibling')->getArgument(0);
        $this->assertSame(\Symfony\Component\DependencyInjection\Definition::class, \get_class($argument));
        $this->assertSame('parentClass', $argument->getClass());

        $properties = $container->getDefinition('sibling')->getProperties();
        $this->assertSame(\Symfony\Component\DependencyInjection\Definition::class, \get_class($properties['prop']));
        $this->assertSame('parentClass', $properties['prop']->getClass());

        $methodCalls = $container->getDefinition('sibling')->getMethodCalls();
        $this->assertSame(\Symfony\Component\DependencyInjection\Definition::class, \get_class($methodCalls[0][1][0]));
     ...
     * @group legacy
     */
    public function testProcessMergeAutowiringTypes(): void
    {
        $container = new ContainerBuilder();
     ...
     * @group legacy
     */
    public function testAliasExistsForBackwardsCompatibility(): void
    {
    ...
    public function testProcessDetectsChildDefinitionIndirectCircularReference(): void
    {
