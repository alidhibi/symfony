...
class EnvVarProcessor implements EnvVarProcessorInterface
{
    ...
    public function getEnv($prefix, $name, \Closure $getEnv)
    {
        ...
        if (false !== $i || 'string' !== $prefix) {
            ...
            if (!$this->container->hasParameter(sprintf('env(%s)', $name))) {
                throw new EnvNotFoundException($name);
            }

            if (null === $env = $this->container->getParameter(sprintf('env(%s)', $name))) {
                return null;
            }
        }
        ...
        if ('bool' === $prefix) {
            return (bool) $this->phpize($env);
        }

        if ('int' === $prefix) {
            if (!is_numeric($env = $this->phpize($env))) {
        ...
        if ('resolve' === $prefix) {
            return preg_replace_callback('/%%|%([^%\s]+)%/', function (array $match) use ($name): string|int|float|bool {
                if (!isset($match[1])) {
                    return '%';
                }
    ...
    private function phpize(int|float|string|bool $value)
    {
