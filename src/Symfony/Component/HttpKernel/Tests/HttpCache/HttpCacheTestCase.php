...
class HttpCacheTestCase extends TestCase
{
    ...
    public function assertTraceContains(string $trace): void
    {
    ...
    public function assertTraceNotContains(string $trace): void
    {
        $traces = $this->cache->getTraces();
        $traces = current($traces);
    ...
    public function request($method, $uri = '/', $server = [], $cookies = [], $esi = false, array $headers = []): void
    {
        if (null === $this->kernel) {
    ...
    {
        $values = [];
        foreach (new \RecursiveIteratorIterator(new \RecursiveDirectoryIterator(sys_get_temp_dir().'/http_cache/md', \RecursiveDirectoryIterator::SKIP_DOTS | \FilesystemIterator::SKIP_DOTS), \RecursiveIteratorIterator::LEAVES_ONLY) as $file) {
    ...
    public function setNextResponse($statusCode = 200, array $headers = [], $body = 'Hello World', \Closure $customizer = null): void
    {
