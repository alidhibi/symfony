...
class InlineFragmentRendererTest extends TestCase
{
     ...
     * @group legacy
     */
    public function testRenderWithObjectsAsAttributesPassedAsObjectsInTheControllerLegacy(): void
    {
        $resolver = $this->getMockBuilder(\Symfony\Component\HttpKernel\Controller\ControllerResolver::class)->setMethods(['getController'])->getMock();
        $resolver
            ->expects($this->once())
            ->method('getController')
            ->willReturn(static fn(\stdClass $object, Bar $object1): \Symfony\Component\HttpFoundation\Response => new Response($object1->getBar()))
        ;
     ...
     * @group legacy
     */
    public function testRenderWithObjectsAsAttributesPassedAsObjectsInTheController(): void
    {
    ...
    public function testExceptionInSubRequestsDoesNotMangleOutputBuffers(): void
    {
        ...
        $controllerResolver
            ->expects($this->once())
            ->method('getController')
            ->willReturn(static function () : never {
                ob_start();
                echo 'bar';
    ...
    public function testLocaleAndFormatAreIsKeptInSubrequest(): void
    {
        $expectedSubRequest = Request::create('/');
        $expectedSubRequest->attributes->set('_format', 'foo');
        $expectedSubRequest->setLocale('fr');
        if ((Request::HEADER_X_FORWARDED_FOR & Request::getTrustedHeaderSet()) !== 0) {
            $expectedSubRequest->headers->set('x-forwarded-for', ['127.0.0.1']);
            $expectedSubRequest->server->set('HTTP_X_FORWARDED_FOR', '127.0.0.1');
        }
