...
class TestSessionListenerTest extends TestCase
{
     ...
     */
    public function testEmptySessionDoesNotSendCookie(): void
    {
        $this->sessionHasBeenStarted();
        $this->sessionIsEmpty();
    ...
    public function testEmptySessionWithNewSessionIdDoesSendCookie(): void
    {
        $this->sessionHasBeenStarted();
        $this->sessionIsEmpty();
     ...
     */
    public function testSessionWithNewSessionIdAndNewCookieDoesNotSendAnotherCookie(string $existing, array $expected): void
    {
        $this->sessionHasBeenStarted();
        $this->sessionIsEmpty();
        $this->fixSessionId('456');
    ...
    public function anotherCookieProvider(): array
    {
        return [
    ...
    private function filterResponse(Request $request, int $type = HttpKernelInterface::MASTER_REQUEST, Response $response = null)
    {
    ...
    private function sessionIsEmpty(): void
    {
        $this->session->expects($this->once())
            ->method('isEmpty')
            ->willReturn(true);
    ...
    private function fixSessionId(string $sessionId): void
    {
