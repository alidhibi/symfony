...
abstract class Kernel implements KernelInterface, RebootableInterface, TerminableInterface
{
    ...
    final const VERSION = '3.4.49';

    final const VERSION_ID = 30449;

    final const MAJOR_VERSION = 3;

    final const MINOR_VERSION = 4;

    final const RELEASE_VERSION = 49;

    final const EXTRA_VERSION = '';

    final const END_OF_MAINTENANCE = '11/2020';

    final const END_OF_LIFE = '11/2021';
     ...
     */
    public function loadClassCache($name = 'classes', $extension = '.php'): void
    {
     ...
     */
    public function setClassCache(array $classes): void
    {
        @trigger_error(__METHOD__.'() is deprecated since Symfony 3.3, to be removed in 4.0.', \E_USER_DEPRECATED);
    ...
    protected function initializeBundles()
    {
        ...
        foreach ($this->registerBundles() as $bundle) {
            ...
            if (($parentName = $bundle->getParent()) !== '' && ($parentName = $bundle->getParent()) !== '0') {
        ...
        // look for orphans
        if ($directChildren !== [] && \count($diff = array_diff_key($directChildren, $this->bundles))) {
    ...
    protected function initializeContainer()
    {
        ...
        if ($collectDeprecations = $this->debug && !\defined('PHPUNIT_COMPOSER_INSTALL')) {
            $collectedLogs = [];
            $previousHandler = set_error_handler(static function ($type, $message, $file, $line) use (&$collectedLogs, &$previousHandler) : ?bool {
    ...
    protected function prepareContainer(ContainerBuilder $container)
    {
        ...
        foreach ($this->bundles as $bundle) {
            if (($extension = $bundle->getContainerExtension()) !== null) {
