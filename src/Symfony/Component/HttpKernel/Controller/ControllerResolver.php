...
class ControllerResolver implements ArgumentResolverInterface, ControllerResolverInterface
{
     ...
     *
     */
    private readonly bool $supportsVariadic;

    /**
     * If scalar types exists.
     *
     */
    private readonly bool $supportsScalarTypes;
    ...
    public function getController(Request $request)
    {
        ...
        try {
            ...
            throw new \InvalidArgumentException(sprintf('The controller for URI "%s" is not callable: ', $request->getPathInfo()).$invalidArgumentException->getMessage(), 0, $invalidArgumentException);
        }
        ...
        foreach ($parameters as $param) {
            if (\array_key_exists($param->name, $attributes)) {
                if ($this->supportsVariadic && $param->isVariadic() && \is_array($attributes[$param->name])) {
                    $arguments = [...$arguments, ...array_values($attributes[$param->name])];
                } else {
