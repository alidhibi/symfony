...
class ResponseCacheStrategy implements ResponseCacheStrategyInterface
{
     ...
     */
    private static array $overrideDirectives = ['private', 'no-cache', 'no-store', 'no-transform', 'must-revalidate', 'proxy-revalidate'];

    /**
     * Cache-Control headers that are sent to the final response if they appear in ALL of the responses.
     */
    private static array $inheritDirectives = ['public', 'immutable'];
    ...
    private function willMakeFinalResponseUncacheable(Response $response): bool
    {
        ...
        return !$response->headers->has('Expires');
    }
     ...
     * @param int      $age
     */
    private function storeRelativeAgeDirective(string $directive, $value, $age): void
    {
        if (null === $value) {
