...
class DumpDataCollector extends DataCollector implements DataDumperInterface
{
    ...
    private int|float $clonesIndex = 0;
    ...
    private \Symfony\Component\VarDumper\Dumper\DataDumperInterface|null|\Symfony\Component\VarDumper\Dumper\HtmlDumper|\Symfony\Component\VarDumper\Dumper\CliDumper $dumper = null;

    private readonly bool $dumperIsInjected;
    ...
    public function __construct(Stopwatch $stopwatch = null, $fileLinkFormat = null, $charset = null, RequestStack $requestStack = null, DataDumperInterface $dumper = null)
    {
        ...
        $this->dumper = $dumper;
        $this->dumperIsInjected = $dumper instanceof \Symfony\Component\VarDumper\Dumper\DataDumperInterface;
    }
    ...
    {
        if ($this->stopwatch instanceof \Symfony\Component\Stopwatch\Stopwatch) {
        ...
        for ($i = 1; $i < 7; ++$i) {
            if (isset($trace[$i]['class'], $trace[$i]['function'])
                ...
                && \Symfony\Component\VarDumper\VarDumper::class === $trace[$i]['class']
            ) {
                $file = $trace[$i]['file'];
                $line = $trace[$i]['line'];
        ...
        if ($this->dumper instanceof \Symfony\Component\VarDumper\Dumper\DataDumperInterface) {
            $this->doDump($data, $name, $file, $line);
        }

        $this->data[] = ['data' => $data, 'name' => $name, 'file' => $file, 'line' => $line, 'fileExcerpt' => $fileExcerpt];
        ++$this->dataCount;
    ...
    private function doDump($data, string $name, $file, string $line): void
    {
