...
class SlotsHelperTest extends TestCase
{
    ...
    public function testStartStop(): void
    {
        ...
        try {
            ...
            $helper->stop();
            $this->assertInstanceOf('\InvalidArgumentException', $exception, '->start() throws an InvalidArgumentException if a slot with the same name is already started');
            $this->assertEquals('A slot named "bar" is already started.', $exception->getMessage(), '->start() throws an InvalidArgumentException if a slot with the same name is already started');
        }
        ...
        try {
            ...
            $this->assertInstanceOf('\LogicException', $exception, '->stop() throws an LogicException if no slot is started');
            $this->assertEquals('No slot started.', $exception->getMessage(), '->stop() throws an LogicException if no slot is started');
        }
    }
}
