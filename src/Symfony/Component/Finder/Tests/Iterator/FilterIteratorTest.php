...
{
    public function testFilterFilesystemIterators(): void
    {
        $i = new \FilesystemIterator($this->toAbsolute());

        // it is expected that there are test.py test.php in the tmpDir
        $i = $this->getMockForAbstractClass(\Symfony\Component\Finder\Iterator\FilterIterator::class, [$i]);
        $i->expects($this->any())
            ->method('accept')
            ->willReturnCallback(static fn(): bool => (bool) preg_match('/\.php/', (string) $i->current())
        );
