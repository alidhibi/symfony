...
{
    final const KB_BYTES = 1000;

    final const MB_BYTES = 1_000_000;
    ...
    public function validate($value, Constraint $constraint): void
    {
        ...
        if ($constraint->mimeTypes) {
            ...
            foreach ($mimeTypes as $mimeType) {
                ...
                if (($discrete = strstr($mimeType, '/*', true)) && strstr($mime, '/', true) === $discrete) {
                    return;
                }
            }
     ...
     */
    private function factorizeSizes($size, $limit, $binaryFormat): array
    {
        if ($binaryFormat) {
            $coef = self::MIB_BYTES;
