...
{
    final const NAMESPACE_URI = 'http://symfony.com/schema/routing';

    final const SCHEME_PATH = '/schema/routing/routing-1.0.xsd';
     ...
     */
    public function load($file, $type = null): \Symfony\Component\Routing\RouteCollection
    {
    ...
    private function parseConfigs(\DOMElement $node, $path): array
    {
        ...
        foreach ($node->getElementsByTagNameNS(self::NAMESPACE_URI, '*') as $n) {
            ...
            switch ($n->localName) {
                case 'default':
                    $defaults[$n->getAttribute('key')] = $this->isElementValueNull($n) ? null : $this->parseDefaultsConfig($n, $path);
        ...
        if (($controller = $node->getAttribute('controller')) !== '' && ($controller = $node->getAttribute('controller')) !== '0') {
