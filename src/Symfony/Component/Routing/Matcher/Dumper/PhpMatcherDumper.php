...
class PhpMatcherDumper extends MatcherDumper
{
    ...
    public function dump(array $options = []): string
    {
        $options = array_replace([
            ...
            'base_class' => \Symfony\Component\Routing\Matcher\UrlMatcher::class,
        ], $options);

        // trailing slash support is only enabled if we know how to redirect the user
        $interfaces = class_implements($options['base_class']);
        $supportsRedirections = isset($interfaces[\Symfony\Component\Routing\Matcher\RedirectableUrlMatcherInterface::class]);
...
EOF;
     ...
     */
    private function compileRoutes(RouteCollection $routes, $supportsRedirections): string
    {
        $fetchedHost = false;
    ...
    private function buildStaticPrefixCollection(DumperCollection $collection): \Symfony\Component\Routing\Matcher\Dumper\StaticPrefixCollection
    {
     ...
     * @return string PHP code
     */
    private function compileStaticPrefixRoutes(StaticPrefixCollection $collection, $supportsRedirections, string $ifOrElseIf = 'if'): string|array|null
    {
        $code = '';
    ...
    private function compileRoute(Route $route, $name, $supportsRedirections, $parentPrefix = null)
    {
        ...
        if ($compiledRoute->getPathVariables() === [] && false !== preg_match('#^(.)\^(?P<url>.*?)\$\1#'.('u' === substr($regex, -1) ? 'u' : ''), $regex, $m)) {
...
EOF;
     ...
     */
    private function groupRoutesByHostRegex(RouteCollection $routes): \Symfony\Component\Routing\Matcher\Dumper\DumperCollection
    {
    ...
    private function getExpressionLanguage(): ?\Symfony\Component\ExpressionLanguage\ExpressionLanguage
    {
