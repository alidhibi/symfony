...
class CliDumper extends AbstractDumper
{
     ...
     * {@inheritdoc}
     */
    public function dumpScalar(Cursor $cursor, $type, $value): void
    {
        $this->dumpKey($cursor);
    ...
    public function dumpString(Cursor $cursor, $str, $bin, $cut): void
    {
        ...
        if ('' === $str) {
            ...
            if ((self::DUMP_STRING_LENGTH & $this->flags) !== 0) {
        ...
        if (null !== $key = $cursor->hashKey) {
            ...
            switch ($cursor->hashType) {
                ...
                case Cursor::HASH_INDEXED:
                    if ((self::DUMP_LIGHT_ARRAY & $this->flags) !== 0) {
                        break;
                    }
     ...
     */
    protected function style($style, array $value, array $attr = [])
    {
        if (null === $this->colors) {
            $this->colors = $this->supportsColors();
        }
        ...
        $startCchr = $this->colors ? sprintf('[m[%sm', $this->styles['default']) : '';
        $endCchr = $this->colors ? sprintf('[m[%sm', $style) : '';
        $value = preg_replace_callback(static::$controlCharsRx, static function (array $c) use ($map, $startCchr, $endCchr) : string {
            $s = $startCchr;
            $c = $c[$i = 0];
            do {
        ...
        if ($this->colors) {
            $value = $cchrCount && "\033" === $value[0] ? substr($value, \strlen($startCchr)) : sprintf('[%sm', $style).$value;

            if ($cchrCount && $endCchr === substr($value, -\strlen($endCchr))) {
                $value = substr($value, 0, -\strlen($endCchr));
            } else {
                $value .= sprintf('[%sm', $this->styles['default']);
            }
        }
