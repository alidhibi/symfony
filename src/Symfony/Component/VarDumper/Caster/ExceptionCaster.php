...
class ExceptionCaster
{
    ...
    public static function castErrorException(\ErrorException $e, array $a, Stub $stub, $isNested): array
    {
    ...
    public static function castThrowingCasterException(ThrowingCasterException $e, array $a, Stub $stub, $isNested): array
    {
    ...
    public static function castSilencedErrorContext(SilencedErrorContext $e, array $a, Stub $stub, $isNested): array
    {
     ...
     * @return mixed[]
     */
    public static function castTraceStub(TraceStub $trace, array $a, Stub $stub, $isNested): array
    {
        if (!$isNested) {
            return $a;
        }
        ...
        for ($j += $trace->numberingOffset - $i++; isset($frames[$i]); ++$i, --$j) {
            ...
            if (isset($f[$prefix.'src'])) {
                foreach ($f[$prefix.'src']->value as $label => $frame) {
                    ...
                    $label = substr_replace($label, sprintf('title=Stack level %s.&', $j), 2, 0);
                }
                ...
                if (isset($label->attr['ellipsis'])) {
                    ...
                    $label = substr_replace($prefix, sprintf('ellipsis-type=class&ellipsis=%s&ellipsis-tail=1&title=Stack level %s.', $label->attr['ellipsis'], $j), 2, 0).$label->value.'()';
                } else {
                    $label = substr_replace($prefix, sprintf('title=Stack level %s.', $j), 2, 0).$label->value.'()';
                }
            } else {
                $label = substr_replace($prefix, sprintf('title=Stack level %s.', $j), 2, 0).$lastCall;
            }
    ...
    public static function castFrameStub(FrameStub $frame, array $a, Stub $stub, $isNested)
    {
        ...
        if (isset($f['file'], $f['line'])) {
            ...
            if (isset(self::$framesCache[$cacheKey])) {
                ...
                self::$framesCache[$cacheKey] = $a[$prefix.'src'] = new EnumStub([sprintf(' ~%s %s', $srcAttr, $srcKey) => $src]);
            }
        }
    ...
    private static function filterExceptionArray($xClass, array $a, string $xPrefix, $filter): array
    {
    ...
    private static function traceUnshift(array &$trace, ?string $class, $file, $line): void
    {
    ...
    private static function extractSource(array $srcLines, $line, $srcContext, $title, $lang, $file = null): \Symfony\Component\VarDumper\Caster\EnumStub
    {
        $srcLines = explode("\n", $srcLines);
        $src = [];
