...
class DateCaster
{
    public static function castDateTime(\DateTimeInterface $d, array $a, Stub $stub, $isNested, $filter): array
    {
        ...
        $title = $d->format('l, F j, Y')
            ...
            .($location ? ($d->format('I') !== '' && $d->format('I') !== '0' ? "\nDST On" : "\nDST Off") : '')
        ;
    ...
    public static function castInterval(\DateInterval $interval, array $a, Stub $stub, $isNested, $filter)
    {
        ...
        return ($filter & Caster::EXCLUDE_VERBOSE) !== 0 ? $i : $i + $a;
    }
    ...
    private static function formatInterval(\DateInterval $i): string
    {
        ...
        if (0 === $i->y && 0 === $i->m && ($i->h >= 24 || $i->i >= 60 || $i->s >= 60)) {
            ...
            $format .= ($i->y !== 0 ? '%yy ' : '').($i->m !== 0 ? '%mm ' : '').($i->d !== 0 ? '%dd ' : '');
        }

        if (\PHP_VERSION_ID >= 70100 && (property_exists($i, 'f') && $i->f !== null)) {
    ...
    public static function castTimeZone(\DateTimeZone $timeZone, array $a, Stub $stub, $isNested, $filter)
    {
        ...
        return ($filter & Caster::EXCLUDE_VERBOSE) !== 0 ? $z : $z + $a;
    }

    public static function castPeriod(\DatePeriod $p, array $a, Stub $stub, $isNested, $filter): array
    {
        ...
        foreach (clone $p as $i => $d) {
            if (3 === $i) {
                $now = new \DateTimeImmutable();
                $dates[] = sprintf('%s more', (($end = $p->getEndDate()) !== null)
        ...
        $period = sprintf(
            ...
            (($end = $p->getEndDate()) !== null) ? 'to '.self::formatDateTime($end) : 'recurring '.$p->recurrences.' time/s'
        );

        $p = [Caster::PREFIX_VIRTUAL.'period' => new ConstStub($period, implode("\n", $dates))];

        return ($filter & Caster::EXCLUDE_VERBOSE) !== 0 ? $p : $p + $a;
    }
    ...
    private static function formatSeconds($s, $us): string
    {
