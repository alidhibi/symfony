...
class DateCasterTest extends TestCase
{
     ...
     */
    public function testDumpDateTime(string $time, string $timezone, string $xDate, int $xTimestamp): void
    {
...
EODUMP;
     ...
     * @dataProvider provideDateTimes
     */
    public function testCastDateTime(string $time, string $timezone, string $xDate, int $xTimestamp, string $xInfos): void
    {
        if ((\defined('HHVM_VERSION_ID') || \PHP_VERSION_ID <= 50509) && preg_match('/[-+]\d{2}:\d{2}/', $timezone)) {
            $this->markTestSkipped('DateTimeZone GMT offsets are supported since 5.5.10. See https://github.com/facebook/hhvm/issues/5875 for HHVM.');
        }
...
EODUMP;
    ...
    public function provideDateTimes(): array
    {
        return [
            ['2017-04-30 00:00:00.000000', 'Europe/Zurich', '2017-04-30 00:00:00.0 Europe/Zurich (+02:00)', 1_493_503_200, 'Sunday, April 30, 2017%Afrom now%ADST On'],
            ['2017-12-31 00:00:00.000000', 'Europe/Zurich', '2017-12-31 00:00:00.0 Europe/Zurich (+01:00)', 1_514_674_800, 'Sunday, December 31, 2017%Afrom now%ADST Off'],
            ['2017-04-30 00:00:00.000000', '+02:00', '2017-04-30 00:00:00.0 +02:00', 1_493_503_200, 'Sunday, April 30, 2017%Afrom now'],

            ['2017-04-30 00:00:00.100000', '+00:00', '2017-04-30 00:00:00.100 +00:00', 1_493_510_400, 'Sunday, April 30, 2017%Afrom now'],
            ['2017-04-30 00:00:00.120000', '+00:00', '2017-04-30 00:00:00.120 +00:00', 1_493_510_400, 'Sunday, April 30, 2017%Afrom now'],
            ['2017-04-30 00:00:00.123000', '+00:00', '2017-04-30 00:00:00.123 +00:00', 1_493_510_400, 'Sunday, April 30, 2017%Afrom now'],
            ['2017-04-30 00:00:00.123400', '+00:00', '2017-04-30 00:00:00.123400 +00:00', 1_493_510_400, 'Sunday, April 30, 2017%Afrom now'],
            ['2017-04-30 00:00:00.123450', '+00:00', '2017-04-30 00:00:00.123450 +00:00', 1_493_510_400, 'Sunday, April 30, 2017%Afrom now'],
            ['2017-04-30 00:00:00.123456', '+00:00', '2017-04-30 00:00:00.123456 +00:00', 1_493_510_400, 'Sunday, April 30, 2017%Afrom now'],
        ];
    }
...
EODUMP;
     ...
     */
    public function testDumpInterval(string $intervalSpec, int|float $ms, int $invert, string $expected): void
    {
        if ($ms && \PHP_VERSION_ID >= 70200 && PHP_VERSION_ID <= 70200) {
...
EODUMP;
     ...
     * @dataProvider provideIntervals
     */
    public function testDumpIntervalExcludingVerbosity(string $intervalSpec, int|float $ms, int $invert, string $expected): void
    {
        if ($ms && \PHP_VERSION_ID >= 70200 && PHP_VERSION_ID <= 70200) {
            $this->markTestSkipped('Skipped on 7.2 before rc4 because of php bug #75354.');
        }
...
EODUMP;
     ...
     * @dataProvider provideIntervals
     */
    public function testCastInterval(string $intervalSpec, int|float $ms, int $invert, string $xInterval, ?string $xSeconds): void
    {
        if ($ms && \PHP_VERSION_ID >= 70200 && PHP_VERSION_ID <= 70200) {
            $this->markTestSkipped('Skipped on 7.2 before rc4 because of php bug #75354.');
        }
...
EODUMP;
    ...
    public function provideIntervals(): array
    {
        ...
        $ms = ($withMs = \PHP_VERSION_ID >= 70100 && (property_exists($i, 'f') && $i->f !== null)) ? '.0' : '';
     ...
     */
    public function testDumpTimeZone(string $timezone, string $expected): void
    {
...
EODUMP;
     ...
     * @dataProvider provideTimeZones
     */
    public function testDumpTimeZoneExcludingVerbosity(string $timezone, string $expected): void
    {
        if ((\defined('HHVM_VERSION_ID') || \PHP_VERSION_ID <= 50509) && !preg_match('/\w+\/\w+/', $timezone)) {
            $this->markTestSkipped('DateTimeZone GMT offsets are supported since 5.5.10. See https://github.com/facebook/hhvm/issues/5875 for HHVM.');
        }
...
EODUMP;
     ...
     * @dataProvider provideTimeZones
     */
    public function testCastTimeZone(string $timezone, string $xTimezone, string $xRegion): void
    {
        if ((\defined('HHVM_VERSION_ID') || \PHP_VERSION_ID <= 50509) && !preg_match('/\w+\/\w+/', $timezone)) {
            $this->markTestSkipped('DateTimeZone GMT offsets are supported since 5.5.10. See https://github.com/facebook/hhvm/issues/5875 for HHVM.');
        }
...
EODUMP;
     ...
     */
    public function testDumpPeriod(string $start, string $interval, string|int $end, int $options, string $expected): void
    {
...
EODUMP;
     ...
     * @dataProvider providePeriods
     */
    public function testCastPeriod(string $start, string $interval, string|int $end, int $options, string $xPeriod, string $xDates): void
    {
        if (\defined('HHVM_VERSION_ID') || \PHP_VERSION_ID < 50620 || (\PHP_VERSION_ID >= 70000 && \PHP_VERSION_ID < 70005)) {
            $this->markTestSkipped();
        }
...
EODUMP;
    ...
    {
        $i = new \DateInterval('PT0S');
        $ms = \PHP_VERSION_ID >= 70100 && (property_exists($i, 'f') && $i->f !== null) ? '.0' : '';
        ...
        return [
            ...
            ['2017-01-01', 'P1DT1H', '2017-01-03', 0, sprintf('every + 1d 01:00:00%s, from 2017-01-01 00:00:00.0 (included) to 2017-01-03 00:00:00.0', $ms), '1) 2017-01-01 00:00:00.0%a2) 2017-01-02 01:00:00.0'],
            ['2017-01-01', 'P1DT1H', 1, 0, sprintf('every + 1d 01:00:00%s, from 2017-01-01 00:00:00.0 (included) recurring 2 time/s', $ms), '1) 2017-01-01 00:00:00.0%a2) 2017-01-02 01:00:00.0'],
    ...
    private function createInterval(string $intervalSpec, int|float $ms, int $invert): \DateInterval
    {
        $interval = new \DateInterval($intervalSpec);
        if (\PHP_VERSION_ID >= 70100 && (property_exists($interval, 'f') && $interval->f !== null)) {
            $interval->f = $ms;
        }
