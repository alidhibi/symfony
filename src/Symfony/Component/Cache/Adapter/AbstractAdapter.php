...
abstract class AbstractAdapter implements AdapterInterface, LoggerAwareInterface, ResettableInterface
{
    ...
    private ?\Closure $mergeByLifetime = null;
    ...
    protected function __construct($namespace = '', $defaultLifetime = 0)
    {
        ...
        $getId = fn($key) => $this->getId((string) $key);
        $this->mergeByLifetime = \Closure::bind(
            static function ($deferred, $namespace, array &$expiredIds) use ($getId, $defaultLifetime): array {
                $byLifetime = [];
                $now = time();
                $expiredIds = [];
                ...
                foreach ($deferred as $key => $item) {
                    if (null === $item->expiry) {
                        $byLifetime[max($defaultLifetime, 0)][$getId($key)] = $item->value;
    ...
    public function getItem($key)
    {
        ...
        try {
            ...
            CacheItem::log('Failed to fetch key "{key}"', $this->logger, ['key' => $key, 'exception' => $exception]);
        }
    ...
    public function getItems(array $keys = [])
    {
        ...
        try {
            ...
            CacheItem::log('Failed to fetch requested items', $this->logger, ['keys' => $keys, 'exception' => $exception]);
            $items = [];
        }
        ...
        foreach ($byLifetime as $lifetime => $values) {
            ...
            if (\is_array($e) || 1 === \count($values)) {
                foreach (\is_array($e) ? $e : array_keys($values) as $id) {
                    ...
                    CacheItem::log('Failed to save key "{key}" ({type})', $this->logger, ['key' => substr($id, \strlen($this->namespace)), 'type' => $type, 'exception' => $e instanceof \Exception ? $e : null]);
                }
            } else {
    ...
    private function generateItems($items, array &$keys)
    {
        ...
        try {
            ...
            CacheItem::log('Failed to fetch requested items', $this->logger, ['keys' => array_values($keys), 'exception' => $exception]);
        }
