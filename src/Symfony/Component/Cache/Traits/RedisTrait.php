...
trait RedisTrait
{
     ...
     */
    private function init($redisClient, $namespace = '', $defaultLifetime = 0): void
    {
     ...
     */
    public static function createConnection($dsn, array $options = []): \Predis\Client|\Redis|\Symfony\Component\Cache\Traits\RedisProxy
    {
        if (0 !== strpos($dsn, 'redis://')) {
            throw new InvalidArgumentException(sprintf('Invalid Redis DSN: "%s" does not start with "redis://".', $dsn));
        }

        $params = preg_replace_callback('#^redis://(?:(?:[^:@]*+:)?([^@]*+)@)?#', static function (array $m) use (&$auth) : string {
            if (isset($m[1])) {
                $auth = $m[1];
            }
            return 'file://';
        ...
        if (is_a($class, \Redis::class, true)) {
            ...
            $initializer = static function ($redis) use ($connect, $params, $dsn, $auth) : bool {
                try {
                    ...
                    throw new InvalidArgumentException(sprintf('Redis connection "%s" failed: ', $dsn).$redisException->getMessage());
                }
                return true;
            };
     ...
     * {@inheritdoc}
     */
    protected function doClear(array $namespace): bool
    {
        $cleared = true;
        $hosts = [$this->redis];
    ...
    protected function doSave(array $values, $lifetime): array
    {
        ...
        $results = $this->pipeline(static function () use ($serialized, $lifetime) {
    ...
    private function pipeline(\Closure $generator)
    {
        ...
        if ($this->redis instanceof \RedisCluster || ($this->redis instanceof \Predis\Client && $this->redis->getConnection() instanceof RedisCluster)) {
            ...
            $results = $this->redis->pipeline(static function ($redis) use ($generator, &$ids) : void {
                foreach ($generator() as $command => $args) {
