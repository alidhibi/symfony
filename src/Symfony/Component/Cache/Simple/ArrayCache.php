...
class ArrayCache implements CacheInterface, LoggerAwareInterface, ResettableInterface
{
    ...
    public function getMultiple($keys, $default = null)
    {
        ...
        return $this->generateItems($keys, time(), static fn($k, $v, $hit) => $hit ? $v : $default);
    }
        ...
        if ($this->storeSerialized) {
            foreach ($valuesArray as $key => $value) {
                try {
                    ...
                    CacheItem::log('Failed to save key "{key}" ({type})', $this->logger, ['key' => $key, 'type' => $type, 'exception' => $e]);
    ...
    private function normalizeTtl($ttl): int|bool
    {
        if (null === $ttl) {
