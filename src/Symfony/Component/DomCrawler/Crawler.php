...
class Crawler implements \Countable, \IteratorAggregate
{
    ...
    public function addHtmlContent($content, $charset = 'UTF-8'): void
    {
        ...
        if (\count($base) && !empty($baseHref)) {
            if ($this->baseHref !== '' && $this->baseHref !== '0') {
    ...
    public function children()
    {
        ...
        return $this->createSubCrawler($node !== null ? $this->sibling($node) : []);
    }
    ...
    public function extract($attributes): array
    {
        ...
        foreach ($this->nodes as $node) {
            ...
            foreach ($attributes as $attribute) {
                $elements[] = '_text' === $attribute ? $node->nodeValue : $node->getAttribute($attribute);
            }
     ...
     */
    public function link($method = 'get'): \Symfony\Component\DomCrawler\Link
    {
        if (!$this->nodes) {
            throw new \InvalidArgumentException('The current node list is empty.');
        }
     ...
     */
    public function image(): \Symfony\Component\DomCrawler\Image
    {
        if (\count($this) === 0) {
            throw new \InvalidArgumentException('The current node list is empty.');
        }
     ...
     * @throws \InvalidArgumentException If the current node list is empty or the selected node is not instance of DOMElement
     */
    public function form(array $values = null, $method = null): \Symfony\Component\DomCrawler\Form
    {
        if (!$this->nodes) {
            throw new \InvalidArgumentException('The current node list is empty.');
        }
     ...
     */
    public static function xpathLiteral($s): string
    {
        if (false === strpos($s, "'")) {
            return sprintf("'%s'", $s);
        }
    ...
    private function discoverNamespace(\DOMXPath $domxpath, $prefix)
    {
        ...
        return (($node = $namespaces->item(0)) !== null) ? $node->nodeValue : null;
    }
     ...
     *
     */
    private function findNamespacePrefixes($xpath): array
    {
