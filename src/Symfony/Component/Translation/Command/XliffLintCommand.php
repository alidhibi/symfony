...
class XliffLintCommand extends Command
{
    ...
    private ?bool $displayCorrectFiles = null;

    private $directoryIteratorProvider;

    private $isReadableProvider;

    public function __construct(string $name = null, $directoryIteratorProvider = null, $isReadableProvider = null)
    {
...
EOF
     ...
     * @return mixed[]
     */
    private function validate($content, $file = null): array
    {
        ...
        $errorMessages = array_map(static fn($error): array => [
            'line' => $error->line,
            'column' => $error->column,
    ...
    private function displayTxt(SymfonyStyle $io, array $filesInfo): int
    {
        ...
        foreach ($filesInfo as $info) {
            if ($info['valid'] && $this->displayCorrectFiles) {
                ...
                $io->listing(array_map(static fn($error) => -1 === $error['line'] ? $error['message'] : sprintf('Line %d, Column %d: %s', $error['line'], $error['column'], $error['message']), $info['messages']));
            }
        }
