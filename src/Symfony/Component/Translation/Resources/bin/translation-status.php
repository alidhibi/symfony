...
foreach ($config['original_files'] as $originalFilePath) {
    ...
    $totalMissingTranslations += array_sum(array_map(static fn($translation): int => count($translation['missingKeys']), array_values($translationStatus)));
...
/**
 * @return array<mixed, array<'missingKeys'|'total'|'translated', int|mixed[]>>
 */
...
function printTranslationStatus($originalFilePath, $translationStatus, $verboseOutput): void
{
...
function printTable($translations, $verboseOutput): void
{
