...
class UserPasswordEncoderCommand extends ContainerAwareCommand
{
    ...
    private ?\Symfony\Component\Security\Core\Encoder\EncoderFactoryInterface $encoderFactory = null;

    private array $userClasses;

    public function __construct(EncoderFactoryInterface $encoderFactory = null, array $userClasses = [])
    {
        if (!$encoderFactory instanceof \Symfony\Component\Security\Core\Encoder\EncoderFactoryInterface) {
...
EOF
     ...
     * @return Question
     */
    private function createPasswordQuestion(): static
    {
    ...
    private function getUserClass(InputInterface $input, SymfonyStyle $io)
    {
        ...
        if ($this->userClasses === []) {
            if (!$this->encoderFactory instanceof \Symfony\Component\Security\Core\Encoder\EncoderFactoryInterface) {
