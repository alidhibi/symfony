...
class MainConfiguration implements ConfigurationInterface
{
    ...
    private readonly array $userProviderFactories;
        ...
        $rootNode
            ->beforeNormalization()
                ->ifTrue(static function (array $v) : bool {
                    ...
                    return !isset($v['access_decision_manager']['strategy']) && !isset($v['access_decision_manager']['service']);
                })
            ...
            ->children()
                ...
                ->arrayNode('access_decision_manager')
                    ...
                    ->end()
                    ->validate()
                        ->ifTrue(static fn($v): bool => isset($v['strategy']) && isset($v['service']))
    ...
    private function addAclSection(ArrayNodeDefinition $rootNode): void
    {
        $rootNode
            ->children()
                ->arrayNode('acl')
            ...
            ->children()
                ->arrayNode('role_hierarchy')
                    ...
                    ->prototype('array')
                        ...
                        ->beforeNormalization()->ifString()->then(static fn($v): array => ['value' => $v])->end()
                        ->beforeNormalization()
                            ->ifTrue(static fn($v): bool => \is_array($v) && isset($v['value']))
                            ->then(static fn($v): array|false => preg_split('/\s*,\s*/', $v['value']))
                        ->end()
                ...
                ->arrayNode('access_control')
                    ...
                    ->prototype('array')
                        ...
                        ->children()
                            ...
                            ->end()
                            ->arrayNode('methods')
                                ->beforeNormalization()->ifString()->then(static fn($v): array|false => preg_split('/\s*,\s*/', $v))->end()
                                ->prototype('scalar')->end()
                            ->end()
    ...
    private function addFirewallsSection(ArrayNodeDefinition $rootNode, array $factories): void
    {
            ...
            ->arrayNode('logout')
                ...
                ->children()
                    ->arrayNode('delete_cookies')
                        ->beforeNormalization()
                            ->ifTrue(static fn($v): bool => \is_array($v) && \is_int(key($v)))
                            ->then(static fn($v): array => array_map(static fn($v): array => ['name' => $v], $v))
                        ->end()
                        ->useAttributeAsKey('name')
                        ->prototype('array')
                            ->children()
        ...
        $firewallNodeBuilder
            ->end()
            ->validate()
                ->ifTrue(static fn($v): bool => true === $v['security'] && isset($v['pattern']) && !isset($v['request_matcher']))
                ->then(static function (array $firewall) use ($abstractFactoryKeys) : array {
    ...
    private function addProvidersSection(ArrayNodeDefinition $rootNode): void
    {
                ...
                ->arrayNode('chain')
                    ...
                    ->children()
                        ->arrayNode('providers')
                            ->beforeNormalization()
                                ->ifString()
                                ->then(static fn($v): array|false => preg_split('/\s*,\s*/', $v))
                            ->end()
                            ->prototype('scalar')->end()
                        ->end()
                    ->end()
        ...
        $providerNodeBuilder
            ->validate()
                ->ifTrue(static fn($v): bool => \count($v) > 1)
                ->thenInvalid('You cannot set multiple provider types for the same provider')
            ->end()
            ->validate()
                ->ifTrue(static fn($v): bool => 0 === \count($v))
                ->thenInvalid('You must set a provider definition for the provider.')
            ->end()
        ;
    }

    private function addEncodersSection(ArrayNodeDefinition $rootNode): void
    {
        $rootNode
            ...
            ->children()
                ->arrayNode('encoders')
                    ...
                    ->prototype('array')
                        ->canBeUnset()
                        ->performNoDeepMerging()
                        ->beforeNormalization()->ifString()->then(static fn($v): array => ['algorithm' => $v])->end()
                        ->children()
