...
class SecurityExtension extends Extension
{
    ...
    private array $listenerPositions = ['pre_auth', 'form', 'http', 'remember_me'];

    private array $factories = [];

    private array $userProviderFactories = [];

    private ?\Symfony\Component\Security\Core\Authorization\ExpressionLanguage $expressionLanguage = null;

    private array $logoutOnUserChangeByContextKey = [];

    private array $statelessFirewallKeys = [];
    ...
    private function aclLoad($config, ContainerBuilder $container): void
    {
    ...
    private function configureDbalAclProvider(array $config, ContainerBuilder $container, $loader): void
    {
    ...
    private function createFirewalls($config, ContainerBuilder $container): void
    {
        ...
        $authenticationProviders = array_map(static fn($id): \Symfony\Component\DependencyInjection\Reference => new Reference($id), array_values(array_unique($authenticationProviders)));
        $container
    ...
    private function createFirewall(ContainerBuilder $container, $id, $firewall, &$authenticationProviders, $providerIds, string $configId)
    {
    ...
    {
        if (!$this->expressionLanguage instanceof \Symfony\Component\Security\Core\Authorization\ExpressionLanguage) {
            if (!class_exists('Symfony\Component\ExpressionLanguage\ExpressionLanguage')) {
     ...
     */
    private function getFirstProvider($firewallName, string|array $listenerName, array $providerIds)
    {
