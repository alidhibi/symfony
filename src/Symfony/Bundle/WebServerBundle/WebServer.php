...
class WebServer
{
    final const STARTED = 0;

    final const STOPPED = 1;
    ...
    public function start(WebServerConfig $config, $pidFile = null): int
    {
    ...
    public function getAddress($pidFile = null): bool|string
    {
        $pidFile = $pidFile ?: $this->getDefaultPidFile();
        if (!file_exists($pidFile)) {
            return false;
    ...
    public function isRunning($pidFile = null): bool
    {
        $pidFile = $pidFile ?: $this->getDefaultPidFile();
        if (!file_exists($pidFile)) {
            return false;
     ...
     */
    private function createServerProcess(WebServerConfig $config): \Symfony\Component\Process\Process
    {
        ...
        $process = new Process([$binary, ...$finder->findArguments(), '-dvariables_order=EGPCS', '-S', $config->getAddress(), $config->getRouter()]);
