...
class XmlDescriptor extends Descriptor
{
    ...
    protected function describeContainerAlias(Alias $alias, array $options = [], ContainerBuilder $builder = null)
    {
        ...
        if (!$builder instanceof \Symfony\Component\DependencyInjection\ContainerBuilder) {
     ...
     *
     */
    private function getRouteDocument(Route $route, $name = null): \DOMDocument
    {
        $dom = new \DOMDocument('1.0', 'UTF-8');
        ...
        $originRequirements = $route->getRequirements();
     ...
     *
     */
    private function getContainerServiceDocument($service, $id, ContainerBuilder $builder = null, $showArguments = false): \DOMDocument
    {
        $dom = new \DOMDocument('1.0', 'UTF-8');
     ...
     *
     */
    private function getContainerServicesDocument(ContainerBuilder $builder, $tag = null, bool $showPrivate = false, bool $showArguments = false, $filter = null): \DOMDocument
    {
        $dom = new \DOMDocument('1.0', 'UTF-8');
        $dom->appendChild($containerXML = $dom->createElement('container'));
     ...
     *
     */
    private function getContainerDefinitionDocument(Definition $definition, $id = null, bool $omitTags = false, $showArguments = false): \DOMDocument
    {
        $dom = new \DOMDocument('1.0', 'UTF-8');
     ...
     *
     */
    private function getContainerAliasDocument(Alias $alias, $id = null): \DOMDocument
    {
        $dom = new \DOMDocument('1.0', 'UTF-8');
     ...
     *
     */
    private function getEventDispatcherListenersDocument(EventDispatcherInterface $eventDispatcher, $event = null): \DOMDocument
    {
        $dom = new \DOMDocument('1.0', 'UTF-8');
    ...
    private function getCallableDocument($callable)
    {
        ...
        if (\is_array($callable)) {
            ...
            } elseif (0 !== strpos($callable[1], 'parent::')) {
                $callableXML->setAttribute('name', $callable[1]);
        ...
        if ($callable instanceof \Closure) {
            ...
            if (($class = $r->getClosureScopeClass()) !== null) {
                $callableXML->setAttribute('class', $class->name);
                if ($r->getClosureThis() === null) {
                    $callableXML->setAttribute('static', 'true');
                }
            }
