...
class TextDescriptor extends Descriptor
{
    ...
    protected function describeContainerServices(ContainerBuilder $builder, array $options = [])
    {
        ...
        $title = $showPrivate ? 'Symfony Container Public and Private Services' : 'Symfony Container Public Services';
        ...
        $tableHeaders = ['Service ID', ...$tagsNames, 'Class name'];
        $tableRows = [];
            ...
            if ($definition instanceof Definition) {
                if ($showTag) {
                    foreach ($definition->getTag($showTag) as $key => $tag) {
                        ...
                        $tableRows[] = 0 === $key ? [$serviceId, ...$tagValues, $definition->getClass()] : ['  "', ...$tagValues, ''];
                    }
                } else {
                ...
                $alias = $definition;
                $tableRows[] = array_merge([$styledServiceId, sprintf('alias for "%s"', $alias)], $tagsCount !== 0 ? array_fill(0, $tagsCount, '') : []);
            } else {
                $tableRows[] = array_merge([$styledServiceId, \get_class($definition)], $tagsCount !== 0 ? array_fill(0, $tagsCount, '') : []);
            }
        }
        ...
        if (!$omitTags && ($tags = $definition->getTags())) {
            ...
            foreach ($tags as $tagName => $tagData) {
                foreach ($tagData as $tagParameters) {
                    $parameters = array_map(static fn($key, $value): string => sprintf('<info>%s</info>: %s', $key, $value), array_keys($tagParameters), array_values($tagParameters));
                    $parameters = implode(', ', $parameters);

                    $tagInformation[] = '' === $parameters ? sprintf('%s', $tagName) : sprintf('%s (%s)', $tagName, $parameters);
                }
            }
    ...
    protected function describeContainerAlias(Alias $alias, array $options = [], ContainerBuilder $builder = null)
    {
        ...
        $this->describeContainerDefinition($builder->getDefinition((string) $alias), [...$options, 'id' => (string) $alias]);
    }
