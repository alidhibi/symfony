...
class ControllerNameParser
{
    ...
    public function parse($controller)
    {
        ...
        try {
            ...
            if (($alternative = $this->findAlternative($bundle)) !== '' && ($alternative = $this->findAlternative($bundle)) !== '0') {
                $message .= sprintf(' Did you mean "%s:%s:%s"?', $alternative, $controller, $action);
            }

            throw new \InvalidArgumentException($message, 0, $invalidArgumentException);
        }
     ...
     */
    public function build($controller): string
    {
     ...
     * @return string
     */
    private function findAlternative(string $nonExistentBundleName)
    {
        $bundleNames = array_map(static fn($b) => $b->getName(), $this->kernel->getBundles());
