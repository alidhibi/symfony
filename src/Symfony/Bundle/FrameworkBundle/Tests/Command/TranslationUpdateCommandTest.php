...
class TranslationUpdateCommandTest extends TestCase
{
    ...
    public function testDumpMessagesAndCleanInRootDirectory(): void
    {
    ...
    public function testWriteMessagesInRootDirectory(): void
    {
        $this->fs->remove($this->translationDir);
        $this->translationDir = sys_get_temp_dir().'/'.uniqid('sf2_translation', true);
        $this->fs->mkdir($this->translationDir.'/translations');
    ...
    private function createCommandTester($extractedMessages = [], $loadedMessages = [], HttpKernel\KernelInterface $kernel = null): \Symfony\Component\Console\Tester\CommandTester
    {
     ...
     */
    public function testLegacyUpdateCommand(): void
    {
        $translator = $this->getMockBuilder('Symfony\Component\Translation\Translator')
            ->disableOriginalConstructor()
            ->getMock();
        $extractor = $this->getMockBuilder('Symfony\Component\Translation\Extractor\ExtractorInterface')->getMock();
        $loader = $this->getMockBuilder(\Symfony\Bundle\FrameworkBundle\Translation\TranslationLoader::class)->getMock();
        $writer = $this->getMockBuilder('Symfony\Component\Translation\Writer\TranslationWriter')->getMock();
        $kernel = $this->getMockBuilder('Symfony\Component\HttpKernel\KernelInterface')->getMock();
        $kernel
            ->expects($this->any())
