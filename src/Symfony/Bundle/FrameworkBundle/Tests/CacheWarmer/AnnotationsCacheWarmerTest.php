...
class AnnotationsCacheWarmerTest extends TestCase
{
    ...
    public function testAnnotationsCacheWarmerWithDebugDisabled(): void
    {
    ...
    public function testAnnotationsCacheWarmerWithDebugEnabled(): void
    {
        file_put_contents($this->cacheDir.'/annotations.map', sprintf('<?php return %s;', var_export([__CLASS__], true)));
        $cacheFile = tempnam($this->cacheDir, __FUNCTION__);
        $reader = new AnnotationReader();
    ...
    public function testClassAutoloadException(): void
    {
        ...
        spl_autoload_register($classLoader = static function ($class) use ($annotatedClass) : void {
