...
class FrameworkExtension extends Extension
{
    ...
    private ?string $kernelRootHash = null;
        ...
        if ($this->isConfigEnabled($container, $config['matcher'])) {
            if (isset($config['matcher']['service'])) {
                ...
                if (isset($config['matcher']['ip'])) {
                    $definition->addMethodCall('matchIp', [$config['matcher']['ip']]);
                }

                if (isset($config['matcher']['ips'])) {
                    $definition->addMethodCall('matchIps', [$config['matcher']['ips']]);
                }

                if (isset($config['matcher']['path'])) {
                    $definition->addMethodCall('matchPath', [$config['matcher']['path']]);
                }
            }
        }
    ...
    private function registerWorkflowConfiguration(array $config, ContainerBuilder $container, XmlFileLoader $loader): void
    {
        ...
        foreach ($config['workflows'] as $name => $workflow) {
            ...
            if ($workflow['supports']) {
                ...
                $registryDefinition->addMethodCall('add', [new Reference($workflowId), new Reference($workflow['support_strategy'])]);
            }
    ...
    private function registerTemplatingConfiguration(array $config, ContainerBuilder $container, XmlFileLoader $loader): void
    {
        ...
        if (!empty($config['loaders'])) {
            $loaders = array_map(static fn($loader): \Symfony\Component\DependencyInjection\Reference => new Reference($loader), $config['loaders']);
        ...
        $engines = array_map(static fn($engine): \Symfony\Component\DependencyInjection\Reference => new Reference('templating.engine.'.$engine), $config['engines']);
    ...
    private function registerValidationConfiguration(array $config, ContainerBuilder $container, XmlFileLoader $loader): void
    {
        ...
        if (isset($config['cache']) && $config['cache']) {
            ...
            $validatorBuilder->addMethodCall('setMetadataCache', [new Reference($config['cache'])]);
    ...
    private function registerPropertyInfoConfiguration(ContainerBuilder $container, XmlFileLoader $loader): void
    {
    ...
    private function registerCacheConfiguration(array $config, ContainerBuilder $container): void
    {
        ...
        foreach (['doctrine', 'psr6', 'redis', 'memcached'] as $name) {
