...
class Configuration implements ConfigurationInterface
{
    ...
    public function getConfigTreeBuilder(): \Symfony\Component\Config\Definition\Builder\TreeBuilder
    {
        ...
        $rootNode
            ->beforeNormalization()
                ->ifTrue(static fn($v): bool => !isset($v['assets']) && isset($v['templating']) && class_exists(Package::class))
            ...
            ->children()
                ...
                ->arrayNode('trusted_proxies')
                    ...
                    ->beforeNormalization()
                        ->ifTrue(static fn($v): bool => !\is_array($v) && null !== $v)
                        ->then(static fn($v): array|false => \is_bool($v) ? [] : preg_split('/\s*,\s*/', $v))
                    ->end()
                    ->prototype('scalar')
                        ->validate()
                ...
                ->arrayNode('trusted_hosts')
                    ->beforeNormalization()->ifString()->then(static fn($v): array => [$v])->end()
                    ...
                    ->beforeNormalization()
                        ->always(static function ($v) {
                            if (\is_array($v) && true === $v['enabled']) {
                                ...
                                if (1 === \count($workflows) && isset($workflows['workflows']) && array_keys($workflows['workflows']) !== range(0, \count($workflows) - 1) && array_diff(array_keys($workflows['workflows']), ['audit_trail', 'type', 'marking_store', 'supports', 'support_strategy', 'initial_place', 'places', 'transitions']) !== []) {
                            ...
                            ->prototype('array')
                                ...
                                ->children()
                                    ...
                                    ->arrayNode('marking_store')
                                        ...
                                        ->end()
                                        ->validate()
                                            ->ifTrue(static fn($v): bool => isset($v['type']) && isset($v['service']))
                                            ->thenInvalid('"type" and "service" cannot be used together.')
                                        ->end()
                                        ->validate()
                                            ->ifTrue(static fn($v): bool => !empty($v['arguments']) && isset($v['service']))
                                    ...
                                    ->arrayNode('supports')
                                        ...
                                        ->prototype('scalar')
                                            ->cannotBeEmpty()
                                            ->validate()
                                                ->ifTrue(static fn($v): bool => !class_exists($v) && !interface_exists($v))
                                    ...
                                    ->arrayNode('transitions')
                                        ->beforeNormalization()
                                            ->always()
                                            ->then(static function (array $transitions) {
                ...
                ->arrayNode('request')
                    ...
                    ->children()
                        ->arrayNode('formats')
                            ...
                            ->prototype('array')
                                ->beforeNormalization()
                                    ->ifTrue(static fn($v): bool => \is_array($v) && isset($v['mime_type']))
        ...
        $rootNode
            ->children()
                ->arrayNode('templating')
                    ...
                    ->canBeEnabled()
                    ->beforeNormalization()
                        ->ifTrue(static fn($v): bool => false === $v || \is_array($v) && false === $v['enabled'])
                        ->then(static fn(): array => ['enabled' => false, 'engines' => false])
                    ->end()
                    ->children()
                        ...
                        ->arrayNode('form')
                            ...
                            ->children()
                                ->arrayNode('resources')
                                    ...
                                    ->validate()
                                        ->ifTrue(static fn($v): bool => !\in_array('FrameworkBundle:Form', $v))
                                        ->then(static fn($v): array => array_merge(['FrameworkBundle:Form'], $v))
                                    ->end()
                                ->end()
                            ->end()
                        ->end()
                    ...
                    ->children()
                        ->arrayNode('engines')
                            ...
                            ->canBeUnset()
                            ->beforeNormalization()
                                ->ifTrue(static fn($v): bool => !\is_array($v) && false !== $v)
                                ->then(static fn($v): array => [$v])
                            ->end()
                            ->prototype('scalar')->end()
                        ->end()
                ...
                ->arrayNode('validation')
                    ...
                    ->children()
                        ...
                        ->arrayNode('static_method')
                            ...
                            ->validate()
                                ->ifTrue(static fn($v): bool => !\is_array($v))
                                ->then(static fn($v): array => (array) $v)
                            ->end()
                        ->end()
        ...
        $rootNode
            ->children()
                ->arrayNode('lock')
                    ...
                    ->beforeNormalization()
                        ->ifString()->then(static fn($v): array => ['enabled' => true, 'resources' => $v])
                    ->end()
                    ->beforeNormalization()
                        ->ifTrue(static fn($v): bool => \is_array($v) && !isset($v['enabled']))
                        ->then(static fn($v) => $v + ['enabled' => true])
                    ->end()
                    ->beforeNormalization()
                        ->ifTrue(static fn($v): bool => \is_array($v) && !isset($v['resources']) && !isset($v['resource']))
                        ->then(static function (array $v) : array {
                            $e = $v['enabled'];
                            unset($v['enabled']);
                    ...
                    ->children()
                        ->arrayNode('resources')
                            ...
                            ->beforeNormalization()
                                ->ifString()->then(static fn($v): array => ['default' => $v])
                            ->end()
                            ->beforeNormalization()
                                ->ifTrue(static fn($v): bool => \is_array($v) && array_keys($v) === range(0, \count($v) - 1))
