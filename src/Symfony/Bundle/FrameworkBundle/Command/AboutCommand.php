...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $rows = [
            ...
            ['Root directory', $this->formatPath($kernel->getRootDir(), $kernel->getProjectDir())],
            ['Cache directory', $this->formatPath($kernel->getCacheDir(), $kernel->getProjectDir()).' (<comment>'.$this->formatFileSize($kernel->getCacheDir()).'</>)'],
            ['Log directory', $this->formatPath($kernel->getLogDir(), $kernel->getProjectDir()).' (<comment>'.$this->formatFileSize($kernel->getLogDir()).'</>)'],
            new TableSeparator(),
            ['<info>PHP</>'],
            new TableSeparator(),
            ['Version', \PHP_VERSION],
        ...
        if ($dotenv = $this->getDotenvVars()) {
            $rows = [...$rows, new TableSeparator(), ['<info>Environment (.env)</>'], new TableSeparator(), ...array_map(static fn($value, $name): array => [$name, $value], $dotenv, array_keys($dotenv))];
        }
    ...
    private function formatFileSize($path)
    {
        if (is_file($path)) {
            ...
            $size = 0;
            foreach (new \RecursiveIteratorIterator(new \RecursiveDirectoryIterator($path, \RecursiveDirectoryIterator::SKIP_DOTS | \RecursiveDirectoryIterator::FOLLOW_SYMLINKS | \FilesystemIterator::SKIP_DOTS)) as $file) {
                $size += $file->getSize();
            }
        }
