...
class TranslationUpdateCommand extends ContainerAwareCommand
{
     ...
     */
    public function __construct(string $writer = null)
    {
...
EOF
    ...
    protected function execute(InputInterface $input, OutputInterface $output): ?int
    {
        ...
        if (\count($operation->getDomains()) === 0) {
        ...
        if (true === $input->getOption('dump-messages')) {
            ...
            foreach ($operation->getDomains() as $domain) {
                ...
                $list = [...array_diff($allKeys, $newKeys), ...array_map(static fn($id): string => sprintf('<fg=green>%s</>', $id), $newKeys), ...array_map(static fn($id): string => sprintf('<fg=red>%s</>', $id), array_keys($operation->getObsoleteMessages($domain)))];
    ...
    private function filterCatalogue(MessageCatalogue $catalogue, $domain): \Symfony\Component\Translation\MessageCatalogue
    {
