...
{
    private readonly \Symfony\Bundle\WebProfilerBundle\Csp\NonceGenerator $nonceGenerator;
    ...
    {
        return array_reduce(array_keys($directives), static fn($res, $name): string => ('' !== $res ? $res.'; ' : '').sprintf('%s %s', $name, implode(' ', $directives[$name])), '');
    }
     ...
     */
    private function parseDirectives($header): array
    {
        $directives = [];
