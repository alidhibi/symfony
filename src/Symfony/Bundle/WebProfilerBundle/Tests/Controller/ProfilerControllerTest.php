...
class ProfilerControllerTest extends WebTestCase
{
    public function testHomeActionWithProfilerDisabled(): void
    {
        ...
        $controller = new ProfilerController($urlGenerator, $twig, [], null);
     ...
     */
    public function testToolbarActionWithEmptyToken(?string $token): void
    {
        $urlGenerator = $this->getMockBuilder('Symfony\Component\Routing\Generator\UrlGeneratorInterface')->getMock();
        $twig = $this->getMockBuilder('Twig\Environment')->disableOriginalConstructor()->getMock();
        $profiler = $this
        ...
        $controller = new ProfilerController($urlGenerator, $twig, [], $profiler);

        $response = $controller->toolbarAction(Request::create('/_wdt/empty'), $token);
        $this->assertEquals(200, $response->getStatusCode());
    }

    public function getEmptyTokenCases(): array
    {
        return [
            [null],
     ...
     */
    public function testOpeningDisallowedPaths(string $path, bool $isAllowed): void
    {
        $urlGenerator = $this->getMockBuilder('Symfony\Component\Routing\Generator\UrlGeneratorInterface')->getMock();
        $twig = $this->getMockBuilder('Twig\Environment')->disableOriginalConstructor()->getMock();
        $profiler = $this
        ...
        $controller = new ProfilerController($urlGenerator, $twig, [], $profiler, 'bottom', null, __DIR__.'/../..');
    ...
    public function getOpenFileCases(): array
    {
        return [
            ['README.md', true],
            ['composer.json', true],
     ...
     */
    public function testReturns404onTokenNotFound(bool $withCsp): void
    {
        $twig = $this->getMockBuilder('Twig\Environment')->disableOriginalConstructor()->getMock();
        $profiler = $this
            ->getMockBuilder('Symfony\Component\HttpKernel\Profiler\Profiler')
        ...
        $profiler
            ...
            ->method('loadProfile')
            ->willReturnCallback(static fn($token): ?\Symfony\Component\HttpKernel\Profiler\Profile => 'found' == $token ? new Profile($token) : null)
        ;
     ...
     * @dataProvider provideCspVariants
     */
    public function testSearchResultsAction(bool $withCsp): void
    {
        $twig = $this->getMockBuilder('Twig\Environment')->disableOriginalConstructor()->getMock();
        $profiler = $this
            ->getMockBuilder('Symfony\Component\HttpKernel\Profiler\Profiler')
    ...
    public function provideCspVariants(): array
    {
        return [
            [true],
            [false],
    ...
    private function createController($profiler, $twig, bool $withCSP): \Symfony\Bundle\WebProfilerBundle\Controller\ProfilerController
    {
        $urlGenerator = $this->getMockBuilder('Symfony\Component\Routing\Generator\UrlGeneratorInterface')->getMock();
        ...
        if ($withCSP) {
            ...
            return new ProfilerController($urlGenerator, $twig, [], $profiler, 'bottom', new ContentSecurityPolicyHandler($nonceGenerator));
        }

        return new ProfilerController($urlGenerator, $twig, [], $profiler);
    }
}
